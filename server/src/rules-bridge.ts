...
export class RulesBridge {
  ...
  private convertToRulesEngineState(existingState: any): GameState {
    ...
    return {
      ...
      turnNumber: existingState.turnNumber || existingState.turn || 0,
      ...
      zones: existingState.zones,
      manaPool: existingState.manaPool,
  ...
  private convertPlayers(state: any): any[] {
    ...
    return players.map((p: any) => ({
      ...
      hand: state.zones?.[p.id || p.playerId]?.hand || p.hand || [],
      library: state.zones?.[p.id || p.playerId]?.library || p.library || [],
      graveyard: state.zones?.[p.id || p.playerId]?.graveyard || p.graveyard || [],
      battlefield: state.zones?.[p.id || p.playerId]?.battlefield || p.battlefield || [],
      exile: state.zones?.[p.id || p.playerId]?.exile || p.exile || [],
      commandZone: state.commandZone?.[p.id || p.playerId] || p.commandZone || [],
      counters: p.counters || {},
      hasLost: p.hasLost || false,
      commanderDamage: p.commanderDamage,
      manaPool: p.manaPool || state.manaPool?.[p.id || p.playerId],
    }));
  }
