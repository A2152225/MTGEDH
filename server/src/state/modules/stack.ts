    ...
    for (const trigger of otherTriggers) {
      ...
      if (trigger.triggerType === 'creature_etb' && isCreature) {
        ...
        if (creatureTypeMatch) {
          const requiredType = creatureTypeMatch[1].trim();
          const enteringTypeLine = String(token.card?.type_line || '');
          if (!matchesRequiredTypePhrase(enteringTypeLine, requiredType, true)) {
            continue;
          }
        }
...
function executeTriggerEffect(
  ...
  const desc = String(description || "").replace(/[’]/g, "'").toLowerCase();
  ...
  switch (effect.kind) {
    ...
    case 'DamagePermanent': {
      ...
      if (perm) {
        ...
        // Check for damage-received triggers (Brash Taunter, Boros