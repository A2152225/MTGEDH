import type { PlayerID, PlayerRef, PlayerZones, CommanderInfo, KnownCardRef, ClientGameView } from "../types";
...
export function viewFor(ctx: GameContext, viewer: PlayerID, _spectator: boolean): ClientGameView {
  ...
  const filteredBattlefield = state.battlefield.map(perm => {
    ...
    return {
      ...perm,
      card: perm.card,
      ...(typeof effectivePower === "number" && typeof effectiveToughness === "number" ? { effectivePower, effectiveToughness } : {}),
  ...
  for (const p of (state.players as any as PlayerRef[])) {
    const z = zones?.[p.id] ?? { hand: [], handCount: 0, libraryCount: 0, graveyard: [], graveyardCount: 0 };
