...
export function registerAutomationHandlers(
  ...
  socket.on("declareAttackers", async (payload) => {
    const { gameId, attackers } = payload;
    const playerId = socket.data.playerId;
    ...
    console.log(`[Automation] Attackers declared by ${playerId}: ${attackers.length} creatures`);
    
    try {
      // Process attacker declaration as a decision
      const result = await processAttackers(gameId, playerId, attackers);
      
      if (!result.success) {
        socket.emit("error", { message: result.error || "Invalid attacker declaration" });
        return;
      }
      ...
      io.to(gameId).emit("combatState", {
        ...
        attackers: attackers.map(a => ({
          permanentId: a.attackerId,
          defendingPlayer: a.defendingPlayer,
        })),
      });
      ...
      console.error("[Automation] Error declaring attackers:", err);
      socket.emit("error", { message: "Failed to declare attackers" });
    }
  });
  ...
  socket.on("declareBlockers", async (payload) => {
    ...
    console.log(`[Automation] Blockers declared by ${playerId}: ${blockers.length} creatures`);
    
    try {
      // Process blocker declaration as a decision
      const result = await processBlockers(gameId, playerId, blockers);
      
      if (!result.success) {
        socket.emit("error", { message: result.error || "Invalid blocker declaration" });
        return;
      }
      ...
      console.error("[Automation] Error declaring blockers:", err);
      socket.emit("error", { message: "Failed to declare blockers" });
    }
  });
...
async function processAttackers(
  gameId: string,
  playerId: string,
  attackers: Array<{ attackerId: string; defendingPlayer: string }>
): Promise<{ success: boolean; error?: string }> {
  try {
    const game = games.get(gameId);
    if (!game || !game.state) {
 ...
 */
async function processBlockers(
  gameId: string,
  playerId: string,
  ...
  try {
    ...
    // Update battlefield with blocking creatures
    const battlefield = game.state.battlefield || [];
