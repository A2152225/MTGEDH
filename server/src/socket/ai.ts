...
type AIResolvedActivatedAbilityAction = {
  permanentId: string;
  cardName: string;
  abilityText: string;
  abilityId?: string;
...
function getAIPlayerLibraryCards(game: any, playerId: PlayerID): any[] {
  ...
  const fallbackLibrary = game?.state?.zones?.[playerId]?.library;
  return Array.isArray(fallbackLibrary) ? fallbackLibrary : [];
}

function getAITopLibraryCard(game: any, playerId: PlayerID): any | null {
  const library = getAIPlayerLibraryCards(game, playerId);
  return library[0] ?? null;
}

function getAISourceCards(game: any, playerId: PlayerID, sourceZone: AISupportedCardSourceZone): any[] {
  if (sourceZone === 'library') {
    return getAIPlayerLibraryCards(game, playerId);
  }

  if (sourceZone === 'exile') {
    const exile = game?.state?.zones?.[playerId]?.exile;
    return Array.isArray(exile) ? exile : [];
  }

  if (sourceZone === 'graveyard') {
    const graveyard = game?.state?.zones?.[playerId]?.graveyard;
...
function calculateResponseSpellPriority(card: any, game: any, playerId: PlayerID): number {
  ...
  if (oracleText.includes('counter target') && stack.length > 0) {
    priority += 120;
  }
  ...
  if (typeLine.includes('creature') && (oracleText.includes('flash') || oracleText.includes('reach'))) {
    priority += 25;
  }
...
async function tryAIResponseActions(
  ...
  const responseContext = getAIActionContext(game);
  if (!canRespond(responseContext, playerId)) {
    return false;
  }
...
function applyAISelectedLandFace(card: any, selectedFaceIndex?: number): void {
  if (!card || typeof selectedFaceIndex !== 'number') {
    return;
  }
  ...
  card.name = selectedCardFace.name;
  card.type_line = selectedCardFace.type_line;
  card.oracle_text = selectedCardFace.oracle_text;
  card.mana_cost = selectedCardFace.mana_cost;
  if (selectedCardFace.image_uris) {
    card.image_uris = selectedCardFace.image_uris;
  }
  card.selectedMDFCFace = selectedFaceIndex;
}

/**
 * Find a playable land in the AI's supported zones.
 * Prefer the top of the library when the same player-side permission model allows it,
 * then fall back to lands in hand.
 */
...
function hasManaAbility(card: any): boolean {
  const nativeOracleText = String(card?.oracle_text || '');
  const oracleText = (card?.oracle_text || '').toLowerCase();
  const typeLine = (card?.type_line || '').toLowerCase();
  const cardName = (card?.name || '').toLowerCase();
  ...
  const nativeActivatedManaAbilityPattern = /(?:^|\r?\n|\.\s+)(?:\{[^}]+\}(?:,?\s*\{[^}]+\})*(?:,?\s*(?:sacrifice|pay|discard|exile|remove|tap|untap)[^:]*?)?|(?:sacrifice|pay|discard|exile|remove|tap|untap)[^:]*?)\s*:\s*add\s+/i;
  if (nativeActivatedManaAbilityPattern.test(nativeOracleText) && !/target/i.test(oracleText)) {
    return true;
  }
...
function snapshotAIAbilityActivationState(
  ...
  function resolveAIPaymentManaAbilityId(game: any, playerId: PlayerID, permanent: any, manaColor: string): string | undefined {
    const normalizedManaColor = String(manaColor || '').toUpperCase();
    ...
    for (const candidate of candidates) {
      ...
      let score = 0;
      if (normalizedManaColor) {
        if (candidateColors.includes(normalizedManaColor)) {
          score += 100;
        } else if (isAnyColorCandidate && normalizedManaColor !== 'C') {
          score += 80;
        } else {
          continue;
        }

        if (candidateColors.length === 1 && candidateColors[0] === normalizedManaColor) {
          score += 15;
        }
      }

      const producedAmount = Number(
        calculateManaProduction(game.state, permanent, String(playerId), normalizedManaColor, candidate.id).totalAmount || 0
      );
      if (producedAmount > 0) {
        score += producedAmount * 25;
      }
      ...
      if (/\bsacrifice\b/i.test(normalizedText)) {
        score -= 12;
      }
  ...
  function selectAINonTapManaActivationAction(
    ...
    const missingColors = Object.entries(parsedCost.colors || {}).flatMap(([color, amount]) => {
      const normalizedColor = String(color || '').toUpperCase();
      const requiredAmount = Number(amount || 0);
      const poolKey = COLOR_IDENTITY_MAP[normalizedColor] || 'colorless';
      const availableAmount = Number(manaPool?.[poolKey] || 0);
      return requiredAmount > availableAmount ? [normalizedColor] : [];
    });
    ...
    for (const candidate of candidates) {
      ...
      if (missingColors.length > 0) {
        const satisfiesNeededColor = missingColors.some((color) => candidateColors.includes(color));
        const satisfiesNeededAnyColor = isAnyColorCandidate && missingColors.some((color) => color !== 'C');
        score += satisfiesNeededColor || satisfiesNeededAnyColor ? 60 : -40;

        if (missingColors.includes('C') && candidateColors.includes('C')) {
          score += 12;
        }
      }

      if (Number(parsedCost.generic || 0) > 0) {
        score += candidateColors.length > 0 ? 10 : 0;
        if (candidateColors.length === 1 && candidateColors[0] === 'C') {
          score += 4;
        }
      }
      ...
      if (!/\bdiscard\b/i.test(normalizedText)) {
        score += 2;
      } else {
        score -= 12;
      }
...
function getPaymentSources(game: any, playerId: PlayerID, cost: { colors: Record<string, number>; generic: number }): Array<{ sourceId: string; produceColor: string; abilityId?: string; count?: number }> {
  ...
  const availableSources: Array<{ perm: any; producedColors: string[]; isLand: boolean }> = [];
  for (const perm of battlefield) {
    if (perm && perm.controller === playerId && !perm.tapped) {
      // Check if this permanent can produce mana
      if (!hasManaAbility(perm.card)) continue;
      ...
      availableSources.push({ perm, producedColors, isLand });
    }
  }
  ...
  for (const [color, needed] of Object.entries(cost.colors)) {
    ...
    while (colorsPaid[color] < needed) {
      ...
      for (const source of availableSources) {
        ...
        score += Math.min(manaAmount, needed - colorsPaid[color]) * 20;
  ...
  const remainingSources = availableSources
    .filter((source) => !usedSourceIds.has(source.perm.id))
    .map((source) => {
      const colorOptions = source.producedColors.length > 0 ? source.producedColors : ['C'];
  ...
  for (const entry of remainingSources) {
  ...
  function buildAIAvailableManaForSpellPayment(
    ...
    for (const source of paymentSources) {
      ...
      const manaInfo = calculateManaProduction(game.state, permanent, String(playerId), source.produceColor, source.abilityId);
  ...
  export async function chooseAISpellPaymentSelections(
    ...
    playerId: PlayerID,
    step: any,
  ): Promise<{ payment: Array<{ permanentId: string; mana: string; count?: number; abilityId?: string }> } | null> {
...
export async function handleAIPriority(
  ...
  try {
    ...
    if (!isAITurn) {
      ...
      if (await tryAIResponseActions(io, gameId, playerId, game)) {
        return;
      }
    ...
    // If there's something on the stack, try player-style responses first.
    if (!stackEmpty) {
      if (await tryAIResponseActions(io, gameId, playerId, game)) {
        return;
      }
    ...
    if (isMainPhase) {
      ...
      const sharedMainPhaseCanAct = canAct(mainPhaseActionContext, playerId);
      if (!sharedMainPhaseCanAct) {
        debug(1, '[AI] Shared canAct reports no player-style main-phase actions; passing priority');
        await executePassPriority(io, gameId, playerId);
        return;
      }
      ...
      if (canAct(mainPhaseActionContext, playerId)) {
        debug(2, '[AI] Shared canAct still reports available actions after AI exhausted supported main-phase choices');
      }
    ...
    if (phase === 'combat') {
      if (step.includes('attackers') || step === 'declare_attackers') {
        ...
        if (alreadyDeclaredAttackers) {
          // Attackers already declared, try response actions before passing priority.
          if (await tryAIResponseActions(io, gameId, playerId, game)) {
            return;
          }
      ...
      if (step.includes('blockers') || step === 'declare_blockers') {
        ...
        if (alreadyDeclared) {
          // Blockers already declared, try response actions before passing priority.
          if (await tryAIResponseActions(io, gameId, playerId, game)) {
            return;
          }
...
async function executeAIPlayLand(
  ...
  playerId: PlayerID,
  cardId: string,
  sourceZone: AISupportedCardSourceZone = 'hand',
  selectedFaceIndex?: number,
): Promise<boolean> {
  const game = ensureGame(gameId);
  if (!game) return false;
  
  debug(1, '[AI] Playing land:', { gameId, playerId, cardId, sourceZone, selectedFaceIndex });
  
  try {
    const sourceCards = getAISourceCards(game, playerId, sourceZone);
    const cardToPlay = sourceCards.find((c: any) => c?.id === cardId) || null;

    if (!cardToPlay) {
      debugWarn(1, '[AI] Land not found in AI source zone:', { gameId, playerId, cardId, sourceZone });
      return false;
    }

    if (sourceZone === 'library' && String(sourceCards[0]?.id || '') !== cardId) {
      debugWarn(1, '[AI] Refusing to play a non-top library card as a land:', { gameId, playerId, cardId });
      return false;
    }
...
async function executeAICastSpell(
  ...
  gameId: string,
  playerId: PlayerID,
  candidate: AICastableSpellCandidate,
): Promise<boolean> {
  const game = ensureGame(gameId);
  if (!game) return false;
  ...
  const faceIndex = candidate.faceIndex;
