...
export function registerResolutionHandlers(io: Server, socket: Socket) {
  ...
  socket.on("submitResolutionResponse", async ({ 
    ...
    // Validate LIBRARY_SEARCH before completing the step.
    // If we complete first, invalid selections (or invalid split assignments) would permanently consume the step.
    if (step.type === ResolutionStepType.LIBRARY_SEARCH) {
      const stepAny = step as any;
      const isMandatory = Boolean(stepAny.mandatory);
      ...
      if (cancelled) {
        ...
        const rawSelections = response.selections as any;
        if (!Array.isArray(rawSelections) || !rawSelections.every((s) => typeof s === 'string')) {
          socket.emit('error', { code: 'INVALID_SELECTION', message: 'Invalid library search selection format' });
          return;
        }
        ...
        const minSelections = Math.max(0, Number(stepAny.minSelections ?? 0));
        let maxSelections = Number(stepAny.maxSelections);
        if (!Number.isFinite(maxSelections)) maxSelections = Infinity;
        ...
        if (selectedIds.length < minSelections || selectedIds.length > maxSelections) {
          socket.emit('error', {
            code: 'INVALID_SELECTION_COUNT',
            message: `Select ${Number.isFinite(maxSelections) && maxSelections !== Infinity
        ...
        for (const id of selectedIds) {
          if (!availableIdSet.has(String(id))) {
            socket.emit('error', { code: 'INVALID_SELECTION', message: 'Selected card is not available for this search' });
            return;
          }
        }
        ...
        if (selectedIds.length > 0) {
          ...
          if (maxTypes && typeof maxTypes === 'object') {
            ...
            for (const card of selectedCards as any[]) {
              ...
              for (const [typeKey, maxAllowed] of Object.entries(maxTypes)) {
                ...
                if (tl.includes(String(typeKey).toLowerCase())) {
                  counts[typeKey] = (counts[typeKey] || 0) + 1;
                  if (counts[typeKey] > maxAllowed) {
                    socket.emit('error', { code: 'INVALID_SELECTION', message: 'Selection violates type constraint' });
                    return;
                  }
                }
              }
          ...
          const requireDifferentNames = stepAny.requireDifferentNames === true;
          if (requireDifferentNames) {
            ...
            for (const card of selectedCards as any[]) {
              ...
              if (!name) continue;
              if (seen.has(name)) {
                socket.emit('error', { code: 'INVALID_SELECTION', message: 'Selection must have different names' });
                return;
              }
              seen.add(name);
            }
        ...
        // Split-destination validation (Cultivate/Kodama's Reach)
        const destination = String(stepAny.destination || '').toLowerCase();
        const isSplitDestination = stepAny.splitDestination === true || destination === 'split';
        if (isSplitDestination) {
          const sa = (response as any).splitAssignments;
          if (!sa || typeof sa !== 'object') {
            socket.emit('error', { code: 'INVALID_SELECTION', message: 'Missing split assignments' });
            return;
          }

          const toBattlefieldRaw = Array.isArray(sa.toBattlefield) ? sa.toBattlefield : [];
          const toHandRaw = Array.isArray(sa.toHand) ? sa.toHand : [];
          const toBattlefield = toBattlefieldRaw.map((s: any) => String(s)).filter(Boolean);
          const toHand = toHandRaw.map((s: any) => String(s)).filter(Boolean);

          const allAssigned = [...toBattlefield, ...toHand];
          const uniqueAssigned = new Set(allAssigned);
          if (uniqueAssigned.size !== allAssigned.length) {
            socket.emit('error', { code: 'INVALID_SELECTION', message: 'Duplicate assignment in split destination' });
            return;
          }

          // Assigned cards must exactly match selected cards.
          const selectedSet = new Set(selectedIds);
          if (allAssigned.length !== selectedIds.length || !allAssigned.every((id) => selectedSet.has(id))) {
            socket.emit('error', { code: 'INVALID_SELECTION', message: 'Split assignments must match selected cards' });
            return;
          }

          const expectedBattlefield = Number(stepAny.toBattlefield);
          const expectedHand = Number(stepAny.toHand);
          if (Number.isFinite(expectedBattlefield) && expectedBattlefield >= 0 && toBattlefield.length !== expectedBattlefield) {
            socket.emit('error', { code: 'INVALID_SELECTION', message: `Must assign exactly ${expectedBattlefield} card(s) to battlefield` });
            return;
          }
          if (Number.isFinite(expectedHand) && expectedHand >= 0 && toHand.length !== expectedHand) {
            socket.emit('error', { code: 'INVALID_SELECTION', message: `Must assign exactly ${expectedHand} card(s) to hand` });
            return;
          }
        }
      }
    ...
    // Validate activation-cost mana/life before completing the step.
    // If we complete first, we could consume a cost-step and then fail due to insufficient mana,
    // leaving the game in a partial-cost state.
    if (!cancelled) {
      const stepAny = step as any;

      const validateManaAndLifeForActivationCost = async (controllerId: string, manaCost: string, lifeToPayForCost: number) => {
        const manaSymbols = String(manaCost || '').match(/\{[WUBRGC0-9X\/P]+\}/gi) || [];
        const manaOnly = manaSymbols
          .filter((s) => s.toUpperCase() !== '{T}' && s.toUpperCase() !== '{Q}')
          .join('');

        if (/\{[^}]*\/P\}/i.test(manaOnly) || /\{[^}]*X[^}]*\}/i.test(manaOnly)) {
          socket.emit('error', { code: 'UNSUPPORTED_COST', message: `Unsupported activation cost (${manaOnly || 'complex cost'})` });
          return false;
        }
        ...
        if (manaOnly) {
          ...
          if (!resolved.ok) {
            socket.emit('error', { code: 'INSUFFICIENT_MANA', message: (resolved as any).error || 'Insufficient mana.' });
            return false;
          }
          const totalAvailable = calculateTotalAvailableMana(pool, undefined);
      ...
      const validateSourceTapRequirement = (controllerId: string, permanentId: string, requiresTap: boolean) => {
        if (!requiresTap) return true;
        ...
        if (String(perm.controller || '') !== String(controllerId)) {
          socket.emit('error', { code: 'NOT_CONTROLLER', message: 'You no longer control that permanent' });
          return false;
        }
        if ((perm as any).tapped) {
          socket.emit('error', { code: 'ALREADY_TAPPED', message: 'Permanent is already tapped' });
          return false;
        }
        return true;
      };

      // TAP_OTHER as activation cost
      if (step.type === ResolutionStepType.TAP_UNTAP_TARGET && stepAny?.tapOtherAbilityAsCost === true) {
        const controllerId = String(pid);
        ...
        const ok = await validateManaAndLifeForActivationCost(controllerId, manaCost, lifeToPayForCost);
        if (!ok) return;
      }

      // DISCARD/EXILE_FROM_HAND as activation cost
      if (step.type === ResolutionStepType.DISCARD_SELECTION && (stepAny?.discardAbilityAsCost === true || stepAny?.exileFromHandAbilityAsCost === true)) {
        const controllerId = String(pid);
        const permanentId = String(stepAny?.permanentId || step.sourceId || '').trim();
        const requiresTap = Boolean(stepAny?.requiresTap);
        const manaCost = String(stepAny?.manaCost || '');
      ...
      // EXILE_FROM_GRAVEYARD as activation cost
      if (step.type === ResolutionStepType.GRAVEYARD_SELECTION && stepAny?.graveyardExileAbilityAsCost === true) {
        const controllerId = String(pid);
        const permanentId = String(stepAny?.permanentId || step.sourceId || '').trim();
        const requiresTap = Boolean(stepAny?.requiresTap);
        const manaCost = String(stepAny?.manaCost || '');
      ...
      // REMOVE_COUNTERS as activation cost
      if (step.type === ResolutionStepType.TARGET_SELECTION && stepAny?.removeCountersAbilityAsCost === true) {
        const controllerId = String(pid);
        const permanentId = String(stepAny?.permanentId || step.sourceId || '').trim();
        const requiresTap = Boolean(stepAny?.requiresTap);
        const manaCost = String(stepAny?.manaCost || '');
      ...
      // SACRIFICE as activation cost
      if (step.type === ResolutionStepType.TARGET_SELECTION && stepAny?.sacrificeAbilityAsCost === true) {
        const controllerId = String(pid);
        const permanentId = String(stepAny?.permanentId || step.sourceId || '').trim();
        const requiresTap = Boolean(stepAny?.requiresTap);
        const manaCost = String(stepAny?.manaCost || '');
      ...
      // RETURN_TO_HAND as activation cost
      if (step.type === ResolutionStepType.TARGET_SELECTION && stepAny?.returnToHandAbilityAsCost === true) {
        const controllerId = String(pid);
        const permanentId = String(stepAny?.permanentId || step.sourceId || '').trim();
        const requiresTap = Boolean(stepAny?.requiresTap);
        const manaCost = String(stepAny?.manaCost || '');
      ...
      // Blight activation-cost steps (validated against pending activation data)
      if (step.type === ResolutionStepType.TARGET_SELECTION && stepAny?.keywordBlight === true && String(stepAny?.keywordBlightStage || '') === 'ability_activation_cost') {
        const controllerId = String(stepAny?.keywordBlightController || pid);
        const activationId = String(stepAny?.keywordBlightActivationId || '').trim();
        const pending = activationId ? (game.state as any)?.pendingBlightAbilityActivations?.[activationId] : undefined;
        if (!pending) {
          socket.emit('error', { code: 'INVALID_STEP', message: 'Pending activation not found' });
          return;
        }
        const permanentId = String(pending.permanentId || '').trim();
...
async function handleStepResponse(
  ...
  if (response.cancelled) {
    ...
    } else if (step.type === ResolutionStepType.TAP_UNTAP_TARGET) {
      // Activation-cost tap flow: cancel should abort the activation.
      if ((step as any)?.tapOtherAbilityAsCost === true) {
        const pid = response.playerId;
        io.to(gameId).emit('chat', {
          id: `m_${Date.now()}`,
          gameId,
          from: 'system',
          message: `${getPlayerName(game, pid)} cancelled an ability activation.`,
          ts: Date.now(),
        });
      }
  ...
  switch (step.type) {
    case ResolutionStepType.OPENING_HAND_ACTIONS: {
      const selectionsRaw = response.selections as any;
      const selectedCardIds: string[] = Array.isArray(selectionsRaw)
        ? selectionsRaw.map((x: any) => String(x)).filter((x: string) => x.length > 0)
        : [];

      const phaseStr = String(game.state?.phase || "").toUpperCase().trim();
      if (phaseStr !== "" && phaseStr !== "PRE_GAME") {
        // If we're no longer pregame, ignore.
        break;
      }
      ...
      if (response.cancelled || selectedCardIds.length === 0) {
        try {
          appendEvent(gameId, (game as any).seq ?? 0, "skipOpeningHandActions", { playerId: pid });
        } catch (e) {
      ...
      if (playedCards.length > 0) {
        io.to(gameId).emit("chat", {
          ...
          gameId,
          from: "system",
          message: `${getPlayerName(game, pid)} begins the game with ${playedCards.join(", ")} on the battlefield.`,
          ts: Date.now(),
        });
      }
    ...
    case ResolutionStepType.TAP_UNTAP_TARGET: {
      ...
      // Activation-cost use should NOT persist a separate tap event; those taps are
      ...
      // Activation-cost tap flow: resume the battlefield ability activation.
      if (isTapOtherAsActivationCost) {
        ...
        const sourcePerm = battlefieldNow.find((p: any) => p && String(p.id) === String(permanentId));
        if (!sourcePerm) {
        ...
        if (remainingNonManaCostText.length > 0) {
          emitToPlayer(io, controllerId, 'error', { code: 'UNSUPPORTED_COST', message: `Unsupported activation cost (${remainingNonManaCostText}).` });
          break;
        }
        ...
        if (manaOnly) {
          ...
          const pool = getOrInitManaPool(game.state, controllerId) as any;
          const paid = validateAndConsumeManaCostFromPool(pool, manaOnly, { logPrefix: '[tapOtherAbilityAsCost]' });
          if (!paid.ok) {
        ...
        // Apply the tap-other portion of the activation cost only after mana/life are validated and paid.
        for (const target of targets) {
          if (!target.tapped) target.tapped = true;
        }

        const tappedPermanentsForCost: string[] = [];
        const tappedSet = new Set<string>();
        for (const tid of targetIds) tappedSet.add(String(tid));

        if (requiresTap) {
          (sourcePerm as any).tapped = true;
          tappedSet.add(String(permanentId));
        }
    ...
    case ResolutionStepType.LIFE_PAYMENT: {
      ...
      const mergedPayment = [
        ...basePayment.filter((p: any) => !(p && typeof p === 'object' && 'lifePayment' in p)),
        { lifePayment },
      ];
    ...
    case ResolutionStepType.MANA_PAYMENT_CHOICE: {
      ...
      if (stepData.spellPaymentRequired === true) {
        ...
        if (response.cancelled) {
          ...
          if (forcedAlternateCostId === 'miracle' && cardId) {
            const zones = (game.state as any)?.zones?.[pid];
            const hand: any[] = Array.isArray(zones?.hand) ? zones.hand : [];
            const cardInHand = hand.find((c: any) => c && String(c.id) === String(cardId));
            if (cardInHand) {
          ...
          io.to(gameId).emit('chat', {
            ...
            gameId,
            from: 'system',
            message: `${getPlayerName(game, pid)} cancelled casting ${cardName}.`,
            ts: Date.now(),
          });
        ...
        const selections = (response.selections || {}) as any;
        const payment = Array.isArray(selections.payment) ? selections.payment : (Array.isArray(response.selections) ? response.selections : undefined);
        ...
        const alternateCostId = forcedAlternateCostId || (selections.alternateCostId != null ? String(selections.alternateCostId) : undefined);
      ...
      // Phyrexian mana payment choice (activated abilities)
    ...
    case ResolutionStepType.MODE_SELECTION: {
      ...
      // ========================================================================
      // MULTI-MODE ACTIVATED ABILITIES (Staff of Domination, Trading Post, etc.)
      // ========================================================================
      if (stepData?.multiModeActivation === true) {
        if (response.cancelled) {
          break;
        }

        const permanentId = String(step.sourceId || stepData.permanentId || '').trim();
        if (!permanentId) {
          emitToPlayer(io, pid as any, 'error', { code: 'NO_PERMANENT', message: 'No permanent id provided' });
          break;
        }

        const battlefield = game.state?.battlefield || [];
        const permanent = battlefield.find((p: any) => p && String(p.id) === permanentId && String(p.controller) === String(pid));
        if (!permanent) {
          emitToPlayer(io, pid as any, 'error', { code: 'PERMANENT_NOT_FOUND', message: 'Permanent not found or not controlled by you' });
          break;
        }

        const card = (permanent as any).card;
        const cardName = String(card?.name || step.sourceName || 'Unknown');
        ...
        if (!multiModeAbility) {
          emitToPlayer(io, pid as any, 'error', { code: 'NO_MULTI_MODE', message: 'Multi-mode ability not found' });
          break;
        }

        const modeIndex = Number.parseInt(String(chosenMode), 10);
        if (!Number.isFinite(modeIndex) || modeIndex < 0 || modeIndex >= (multiModeAbility.modes || []).length) {
          emitToPlayer(io, pid as any, 'error', { code: 'INVALID_MODE', message: 'Invalid mode selection' });
          break;
        }

        const selectedMode = multiModeAbility.modes[modeIndex];
        const costStr = String(selectedMode?.cost || '');
        const requiresTap = /\{\s*T\s*\}/i.test(costStr) || costStr.toLowerCase().includes('tap');

        if (requiresTap && (permanent as any).tapped) {
          emitToPlayer(io, pid as any, 'error', { code: 'ALREADY_TAPPED', message: `${cardName} is already tapped` });
          break;
        }

        // Parse mana cost (exclude {T})
        const manaSymbols = costStr.match(/\{[^}]+\}/g) || [];
        const manaCost = manaSymbols.filter(sym => !/^\{\s*T\s*\}$/i.test(sym)).join('');

        if (manaCost) {
          const pool = getOrInitManaPool(game.state, pid);
          const paid = validateAndConsumeManaCostFromPool(pool as any, manaCost, { logPrefix: '[Resolution:multiModeActivation]' });
          if (!paid.ok) {
            emitToPlayer(io, pid as any, 'error', { code: 'INSUFFICIENT_MANA', message: (paid as any).error || 'Insufficient mana.' });
            break;
          }
        ...
        // Follow-up target selection (handled via Resolution Queue TARGET_SELECTION)
        if (selectedMode?.requiresTarget) {
          const targetType = String(selectedMode?.targetType || 'permanent').toLowerCase();
          ...
          if (validTargets.length === 0) {
            emitToPlayer(io, pid as any, 'error', { code: 'NO_VALID_TARGETS', message: 'No valid targets for this mode' });
            break;
          }
          ...
          ResolutionQueueManager.addStep(gameId, {
            ...
            description: `${cardName}: ${String(selectedMode?.name || 'Choose a target')}`,
            mandatory: false,
            validTargets,
            ...
            multiModeTargeting: true,
            multiModePermanentId: permanentId,
            multiModeCardName: cardName,
            multiModeModeName: String(selectedMode?.name || ''),
            multiModeEffect: String(selectedMode?.effect || ''),
            multiModeTargetType: targetType,
          } as any);
        ...
        // Execute non-targeted mode effects
        let effectExecuted = false;
        const modeNameLower = String(selectedMode?.name || '').toLowerCase();
        const effectLower = String(selectedMode?.effect || '').toLowerCase();

        // Mana abilities
        if (modeNameLower.includes('add mana') || effectLower.includes('add one mana') || effectLower.includes('add {')) {
          ...
          const symbols = effectLower.match(/\{([wubrgc])\}/gi) || [];
          if (symbols.length > 0) {
            const pool = getOrInitManaPool(game.state, pid) as any;
            const poolKeyByCode: Record<string, string> = {
              W: 'white',
              U: 'blue',
              B: 'black',
              R: 'red',
            ...
            for (const sym of symbols) {
              ...
              const key = poolKeyByCode[code];
              if (key) pool[key] = Number(pool[key] || 0) + 1;
            }
            broadcastManaPoolUpdate(io, gameId, pid, pool as any, 'Added mana', game);
            effectExecuted = true;
          }
        }

        // Surveil (legacy surveilChoice flow)
        else if (modeNameLower.includes('surveil') || effectLower.includes('surveil')) {
          const surveilMatch = effectLower.match(/surveil\s*(\d+)/i);
          const surveilAmount = surveilMatch ? Number.parseInt(surveilMatch[1], 10) : 1;

          (game.state as any).pendingSurveil = (game.state as any).pendingSurveil || {};
          ...
          const zones = (game.state as any)?.zones?.[pid];
          if (zones && Array.isArray(zones.library) && zones.library.length > 0) {
            const topCards = zones.library.slice(0, Math.min(surveilAmount, zones.library.length));
            emitToPlayer(io, pid as any, 'surveilChoice', {
              gameId,
              surveilId,
              amount: surveilAmount,
        ...
        // Staff of Domination-ish
        else if (modeNameLower.includes('untap staff')) {
          (permanent as any).tapped = false;
          effectExecuted = true;
        } else if (modeNameLower.includes('draw card')) {
          const zones = (game.state as any)?.zones?.[pid];
          if (zones && Array.isArray(zones.library) && zones.library.length > 0) {
        ...
        } else if (modeNameLower.includes('gain') && modeNameLower.includes('life')) {
          const lifeGain = Number.parseInt(modeNameLower.match(/\d+/)?.[0] || '1', 10);
          (game.state as any).life = (game.state as any).life || {};
        ...
        if (!effectExecuted) {
          emitToPlayer(io, pid as any, 'error', {
            ...
            message: `Effect for "${String(selectedMode?.name || 'mode')}" not yet implemented`,
          });
          break;
        }
        ...
        io.to(gameId).emit('chat', {
          ...
          gameId,
          from: 'system',
          message: `${getPlayerName(game, pid)} activated ${cardName}: ${String(selectedMode?.name || '')}`,
          ts: Date.now(),
        });
        ...
        appendGameEvent(game, gameId, 'ability_activated', {
          playerId: pid,
          permanentId,
          abilityName: String(selectedMode?.name || ''),
          source: cardName,
          ts: Date.now(),
        });
    ...
    case ResolutionStepType.ADDITIONAL_COST_PAYMENT: {
      const stepData = step as any;
      const mandatory = Boolean(stepData.mandatory);
      ...
      if (costType === 'discard') {
        ...
        for (const permId of selectedIds) {
          ...
          if (String(perm?.controller || '') !== String(pid)) continue;
 ...
 */
async function handleDiscardResponse(
  io: Server,
  game: any,
  gameId: string,
  step: ResolutionStep,
  ...
  // Activation-cost flows must validate remaining mana/life BEFORE performing irreversible zone changes.
  ...
  if (isDiscardCostActivation || isExileFromHandCostActivation) {
    ...
    // Combined discard+sacrifice costs are handled across two steps; allow sacrifice text to remain here.
    if (!isExileFromHandCostActivation && (stepAny as any)?.discardAndSacrificeAbilityAsCost === true) {
      remainingNonManaCostText = remainingNonManaCostText.replace(/\bsacrifice\b[^,]*/gi, ' ');
    }

    remainingNonManaCostText = remainingNonManaCostText
      .replace(/[\s,]+/g, ' ')
      .trim();
    ...
    // Validate mana portion of the activation cost (hybrid-aware; conservative: no Phyrexian and no X).
    const manaSymbols = manaCost.match(/\{[WUBRGC0-9X\/P]+\}/gi) || [];
    const manaOnly = manaSymbols
      .filter((s) => s.toUpperCase() !== '{T}' && s.toUpperCase() !== '{Q}')
      .join('');
    ...
    // Ensure the chosen cards are still in the player's actual hand.
    const handNow: any[] = Array.isArray(zones.hand) ? zones.hand : [];
    const handNowIds = new Set(handNow.map((c: any) => String(c?.id || '')).filter(Boolean));
    if (!selections.every((id) => handNowIds.has(String(id)))) {
      emitToPlayer(io, controllerId, 'error', { code: 'CARD_NOT_IN_HAND', message: 'Selected card not found in hand' });
      return;
    }
  }
  ...
  if ((step as any)?.discardAbilityAsCost === true) {
    ...
    try {
      ...
      if (manaOnly) {
        const { getOrInitManaPool, validateAndConsumeManaCostFromPool } = await import('./util.js');
        const pool = getOrInitManaPool(game.state, controllerId) as any;
        const paid = validateAndConsumeManaCostFromPool(pool, manaOnly, { logPrefix: '[discardAbilityAsCost]' });
        if (!paid.ok) {
          emitToPlayer(io, controllerId, 'error', { code: 'INSUFFICIENT_MANA', message: (paid as any).error || 'Insufficient mana.' });
          return;
        }
  ...
  if ((step as any)?.exileFromHandAbilityAsCost === true) {
    try {
      ...
      if (manaOnly) {
        const { getOrInitManaPool, validateAndConsumeManaCostFromPool } = await import('./util.js');
        const pool = getOrInitManaPool(game.state, controllerId) as any;
        const paid = validateAndConsumeManaCostFromPool(pool, manaOnly, { logPrefix: '[exileFromHandAbilityAsCost]' });
        if (!paid.ok) {
          emitToPlayer(io, controllerId, 'error', { code: 'INSUFFICIENT_MANA', message: (paid as any).error || 'Insufficient mana.' });
          return;
        }
...
async function handleTargetSelectionResponse(
  ...
  const emitPendingCastPaymentIfNeeded = (effectId: string, payerId: string) => {
    ...
    if (!existingPaymentStep) {
      ResolutionQueueManager.addStep(gameId, {
        ...
        description: `Pay costs to cast ${pendingCast.cardName}.`,
        mandatory: true,
  ...
  if (response.cancelled) {
    ...
    // Activated-ability additional-cost: sacrifice selection cancellation = activation cancelled.
    if (stepAny?.sacrificeAbilityAsCost === true) {
    ...
    // Activated-ability additional-cost: remove-counters selection cancellation = activation cancelled.
    if (stepAny?.removeCountersAbilityAsCost === true) {
      const controllerId = String(stepAny?.playerId || pid);
      const cardName = String(stepAny?.cardName || step.sourceName || 'Ability');
      io.to(gameId).emit('chat', {
        id: `m_${Date.now()}`,
  ...
  // ========================================================================
  // SACRIFICE AS AN ACTIVATION COST (battlefield abilities)
  // These are enqueued as TARGET_SELECTION steps with sacrificeAbilityAsCost metadata.
  // ========================================================================
  if (stepAny?.sacrificeAbilityAsCost === true) {
    ...
    if (manaOnly) {
      const { resolveManaCostForPoolPayment, getOrInitManaPool, calculateTotalAvailableMana, validateManaPayment, consumeManaFromPool } = await import('./util.js');
      const pool = getOrInitManaPool(game.state, controllerId) as any;
      const resolved = resolveManaCostForPoolPayment(pool, manaOnly);
      if (!resolved.ok) {
        emitToPlayer(io, controllerId, 'error', {
          code: 'INSUFFICIENT_MANA',
          message: (resolved as any).error || 'Insufficient mana.',
        });
        return;
      }
      ...
      consumeMana = async () => {
        consumeManaFromPool(pool, resolved.coloredCost, resolved.genericCost, '[sacrificeAbilityAsCost]');
      };
    }
    ...
    for (const chosenId of selections) {
      const chosenPerm = battlefield.find((p: any) => p && String(p.id) === String(chosenId));
    ...
    for (const chosenId of selections) {
      const chosenPerm = battlefield.find((p: any) => p && String(p.id) === String(chosenId));
      const nm = String(chosenPerm?.card?.name || 'permanent');
      const ok = movePermanentToGraveyard(ctx, String(chosenId), true);
      if (ok) {
        sacrificedNames.push(nm);
    ...
    io.to(gameId).emit('chat', {
      ...
      gameId,
      from: 'system',
      message: `⚡ ${getPlayerName(game, controllerId)} activated ${cardName}'s ability: ${abilityText} (sacrificed ${sacrificedNames.join(', ') || 'permanents'})`,
      ts: Date.now(),
    });
  ...
  // ========================================================================
  // REMOVE COUNTERS AS AN ACTIVATION COST (battlefield abilities)
  // Enqueued as TARGET_SELECTION steps with removeCountersAbilityAsCost metadata.
  // ========================================================================
  if (stepAny?.removeCountersAbilityAsCost === true) {
    const controllerId = String(step.playerId || pid);
    ...
    const manaCost = String(stepAny?.manaCost || '');
    const requiresTap = Boolean(stepAny?.requiresTap);
    const removeCount = Number(stepAny?.removeCount || 1);
    const counterTypeLabel = String(stepAny?.counterType || '').trim();
    const counterKeyCandidatesRaw = Array.isArray(stepAny?.counterKeyCandidates) ? stepAny.counterKeyCandidates : undefined;
    const counterKeyCandidates: string[] = counterKeyCandidatesRaw && counterKeyCandidatesRaw.length
      ? counterKeyCandidatesRaw.map((x: any) => String(x))
      : [counterTypeLabel];
    const removeFromType = String(stepAny?.removeFromType || 'permanent').toLowerCase();
    const mustBeOther = Boolean(stepAny?.mustBeOther);
    ...
    if (!Number.isFinite(removeCount) || removeCount <= 0) {
      emitToPlayer(io, controllerId, 'error', { code: 'UNSUPPORTED_COST', message: 'Invalid counter removal count' });
      return;
    }
    ...
    const chosenTargetId = String(selections?.[0] || '').trim();
    const chosenPerm = battlefield.find((p: any) => p && String(p.id) === chosenTargetId);
    ...
    if (mustBeOther && String(chosenTargetId) === String(sourcePermanentId)) {
    ...
    if (removeFromType !== 'permanent' && !chosenTypeLine.includes(removeFromType)) {
      emitToPlayer(io, controllerId, 'error', { code: 'INVALID_SELECTION', message: `Chosen permanent is not a ${removeFromType}` });
      return;
    }

    // Conservative: only support costs that are (mana symbols and/or {T}/{Q}) plus a remove-counters clause.
    const remainingNonManaCostText = manaCost
      .replace(/\{[^}]+\}/g, ' ')
    ...
    const counters = (chosenPerm as any).counters || {};
    const actualKey = counterKeyCandidates.find((k) => Number(counters?.[k] || 0) >= removeCount);
    const haveN = actualKey ? Number(counters?.[actualKey] || 0) : 0;
    if (!actualKey || haveN < removeCount) {
      emitToPlayer(io, controllerId, 'error', {
        code: 'INSUFFICIENT_COUNTERS',
        message: `Selected permanent doesn't have enough ${counterTypeLabel || 'required'} counter(s). (Need ${removeCount}, have ${haveN})`,
      });
      return;
    }

    // Pay mana portion of the activation cost (conservative: no Phyrexian and no X) BEFORE removing counters.
    const manaSymbols = manaCost.match(/\{[WUBRGC0-9X\/P]+\}/gi) || [];
    const manaOnly = manaSymbols
      .filter((s) => s.toUpperCase() !== '{T}' && s.toUpperCase() !== '{Q}')
      .join('');
    ...
    if (manaOnly) {
      const { getOrInitManaPool, validateAndConsumeManaCostFromPool } = await import('./util.js');
      const pool = getOrInitManaPool(game.state, controllerId) as any;
      const paid = validateAndConsumeManaCostFromPool(pool, manaOnly, { logPrefix: '[removeCountersAbilityAsCost]' });
      if (!paid.ok) {
        emitToPlayer(io, controllerId, 'error', { code: 'INSUFFICIENT_MANA', message: (paid as any).error || 'Insufficient mana.' });
        return;
      }
    ...
    // Remove counters now (this is the chosen cost payment).
    const ctx = {
      state: game.state,
      libraries: (game as any).libraries,
      bumpSeq: typeof (game as any).bumpSeq === 'function' ? (game as any).bumpSeq.bind(game) : (() => {}),
    ...
    updateCounters(ctx as any, String(chosenTargetId), { [actualKey]: -removeCount });
    ...
    io.to(gameId).emit('chat', {
      ...
      gameId,
      from: 'system',
      message: `⚡ ${getPlayerName(game, controllerId)} activated ${cardName}'s ability: ${abilityText} (removed ${removeCount} ${counterTypeLabel || actualKey} counter${removeCount === 1 ? '' : 's'} from ${chosenName})`,
      ts: Date.now(),
    });
  ...
  if (stepAny?.returnToHandAbilityAsCost === true) {
    ...
    if (manaOnly) {
      ...
      consumeMana = async () => {
        consumeManaFromPool(pool, resolved.coloredCost, resolved.genericCost, '[returnToHandAbilityAsCost]');
      };
    }
  ...
  if (stepAny?.tapCreaturesCost === true) {
    ...
    const creatureType = String(stepAny?.creatureType || '').trim();

    if (!cardId) {
      debugWarn(1, `[Resolution] tapCreaturesCost step missing cardId/sourceId`);
      return;
    }

    if (!Number.isFinite(requiredCount) || requiredCount <= 0) {
      debugWarn(1, `[Resolution] tapCreaturesCost invalid requiredCount: ${String(stepAny?.requiredCount)}`);
      return;
    }

    // Enforce exact count for cost payment
    if (selections.length !== requiredCount) {
      debugWarn(1, `[Resolution] tapCreaturesCost wrong selection count: got ${selections.length}, required ${requiredCount}`);
      return;
    }
    ...
    for (const permanentId of selections) {
      ...
      if (!permanent || String(permanent.controller) !== String(pid)) {
        debugWarn(1, `[Resolution] tapCreaturesCost invalid permanent/controller: ${permanentId}`);
        return;
      }
      if ((permanent as any).tapped) {
        debugWarn(1, `[Resolution] tapCreaturesCost selected already tapped permanent: ${permanentId}`);
        return;
      }
      (permanent as any).tapped = true;
      tappedCreatureNames.push(permanent.card?.name || 'Creature');
    }

    const zones = (game.state as any)?.zones?.[pid];
    if (!zones || !Array.isArray(zones.graveyard)) {
      debugWarn(1, `[Resolution] tapCreaturesCost zones/graveyard missing for ${pid}`);
      return;
    }

    const cardIndex = zones.graveyard.findIndex((c: any) => String(c?.id) === cardId);
    if (cardIndex === -1) {
      debugWarn(1, `[Resolution] tapCreaturesCost card not in graveyard: ${cardId}`);
      return;
    }

    const card = zones.graveyard[cardIndex];
    const cardName = String(card?.name || step.sourceName || 'Card');
    ...
    appendEvent(gameId, (game as any).seq ?? 0, 'activateGraveyardAbility', {
      playerId: pid,
      cardId,
      abilityId: String(stepAny?.abilityId || 'tap-creatures-return'),
      tappedCreatureIds: selections,
      creatureType,
      requiredCount,
      destination: 'hand',
    });
  ...
  // ========================================================================
  // MULTI-MODE ABILITY FOLLOW-UP TARGETING (tap/untap/goad etc.)
  // ========================================================================
  if (stepAny?.multiModeTargeting === true) {
    const targetId = String(selections[0] || '').trim();
    if (!targetId) {
      debugWarn(1, `[Resolution] multiModeTargeting missing targetId`);
      return;
    }
    ...
    const targetPerm = battlefield.find((p: any) => p && String(p.id) === targetId);
    if (!targetPerm) {
      debugWarn(1, `[Resolution] multiModeTargeting target not found: ${targetId}`);
      return;
    }

    const cardName = String(stepAny?.multiModeCardName || step.sourceName || 'Ability');
    const permanentId = String(stepAny?.multiModePermanentId || step.sourceId || '');
    const modeName = String(stepAny?.multiModeModeName || '').trim();
    const effect = String(stepAny?.multiModeEffect || '').trim();
    const targetName = String(targetPerm.card?.name || 'permanent');
    ...
    if (modeLower.includes('goad') || effectLower.includes('goad')) {
      const currentTurn = Number((game.state as any)?.turn || 0);
      const expiryTurn = currentTurn + 1;

      (targetPerm as any).goadedBy = Array.isArray((targetPerm as any).goadedBy) ? (targetPerm as any).goadedBy : [];
      ...
      io.to(gameId).emit('chat', {
        ...
        gameId,
        from: 'system',
        message: `${getPlayerName(game, pid)} activated ${cardName}: Goaded ${targetName}.`,
        ts: Date.now(),
      });

      executed = true;
    } else if (modeLower.includes('untap') || effectLower.includes('untap target')) {
      (targetPerm as any).tapped = false;
      io.to(gameId).emit('chat', {
        id: `m_${Date.now()}`,
        gameId,
        from: 'system',
        message: `${getPlayerName(game, pid)} activated ${cardName}: Untapped ${targetName}.`,
        ts: Date.now(),
      });
      executed = true;
    } else if (modeLower.includes('tap') || effectLower.includes('tap target')) {
      (targetPerm as any).tapped = true;
      io.to(gameId).emit('chat', {
        id: `m_${Date.now()}`,
        gameId,
        from: 'system',
        message: `${getPlayerName(game, pid)} activated ${cardName}: Tapped ${targetName}.`,
        ts: Date.now(),
      });
      executed = true;
    }
    ...
    if (!executed) {
      debugWarn(1, `[Resolution] multiModeTargeting effect not implemented (${modeName || effect || 'unknown'})`);
      return;
    }
  ...
  // This is not stack targeting; it's a player-selection input for later effect resolution.
  // ========================================================================
  if (stepAny?.opponentSelection === true) {
    const effectId = String(stepAny?.effectId || step.sourceId || '').trim();
    if (!effectId) {
      debugWarn(1, `[Resolution] Opponent selection step missing effectId/sourceId`);
      return;
    }

    (game.state as any).pendingOpponentSelections = (game.state as any).pendingOpponentSelections || {};
    (game.state as any).pendingOpponentSelections[effectId] = {
      selectedOpponentIds: selections,
      playerId: pid,
      timestamp: Date.now(),
    };

    debug(1, `[Resolution] Opponent selection stored for ${effectId}: ${selections.join(', ') || '(none)'}`);
  ...
  if (
    ...
    // For activated-ability Blight costs, validate remaining mana BEFORE placing counters.
    // Otherwise, we could place -1/-1 counters and then fail the rest of the activation cost.
    if (stage === 'ability_activation_cost') {
      try {
        if (!activationId) {
        ...
        debugWarn(1, `[Resolution] Blight ability_activation_cost: pre-validation failed`, e);
        emitToPlayer(io, controllerId, 'error', { code: 'CANNOT_PAY_COST', message: 'Unable to validate activation cost; please try again.' });
        return;
      }
    }
    ...
    if (stage === 'ability_activation_cost') {
      try {
        ...
        if (manaOnly) {
          const { getOrInitManaPool, validateAndConsumeManaCostFromPool } = await import('./util.js');
          const pool = getOrInitManaPool(game.state, controllerId) as any;
          const paid = validateAndConsumeManaCostFromPool(pool, manaOnly);
          if (!paid.ok) {
            emitToPlayer(io, controllerId, 'error', {
              code: 'INSUFFICIENT_MANA',
              message: (paid as any).error || 'Insufficient mana.',
            });
            return;
          }
        }
...
async function handleLibrarySearchResponse(
  ...
  if (response.cancelled) {
    debug(2, `[Resolution] Library search cancelled by ${pid} (${step.sourceName || 'Library Search'})`);
    return;
  }
...
async function handleOptionChoiceResponse(
  ...
  if (stepData?.miraclePrompt === true) {
    const choiceId = extractId(selectedOption) || (response.cancelled ? 'decline' : null) || 'decline';
    const miracleCardId = String(stepData?.miracleCardId || step.sourceId || '').trim();

    if (!miracleCardId) {
      debugWarn(1, `[Resolution] miraclePrompt: missing miracleCardId/sourceId`);
      return;
    }

    // Find card in hand and clear first-drawn flags if declined.
    const zones = (game.state as any)?.zones?.[playerId];
    const hand: any[] = Array.isArray(zones?.hand) ? zones.hand : [];
    const cardInHand = hand.find((c: any) => c && String(c.id) === miracleCardId);

    if (response.cancelled || choiceId === 'decline') {
      if (cardInHand) {
        delete (cardInHand as any).isFirstDrawnThisTurn;
        delete (cardInHand as any).drawnAt;
      }
      ...
      io.to(gameId).emit('chat', {
        ...
        gameId,
        from: 'system',
        message: `${getPlayerName(game, playerId)} declines to cast ${String(stepData?.sourceName || step.sourceName || 'a card')} for its miracle cost.`,
        ts: Date.now(),
      });
    ...
    await requestCastSpellForSocket(
      io,
      fakeSocket,
      { gameId, cardId: miracleCardId },
      { skipPriorityCheck: true, forcedAlternateCostId: 'miracle' }
    );
...
async function handleGraveyardSelectionResponse(
  ...
  // Activation-cost flows must validate remaining mana/life BEFORE performing irreversible zone changes.
  // Otherwise, we could exile cards and then discover mana can't be paid.
  if ((step as any)?.graveyardExileAbilityAsCost === true) {
    const stepAny = step as any;
    const controllerId = String(pid);
    const permanentId = String(stepAny?.permanentId || step.sourceId || '').trim();
    ...
    // Ensure the chosen cards are still in the player's actual graveyard.
    const gyNow: any[] = Array.isArray(srcZones.graveyard) ? srcZones.graveyard : [];
    const gyNowIds = new Set(gyNow.map((c: any) => String(c?.id || '')).filter(Boolean));
    if (!selectedCardIds.every((id) => gyNowIds.has(String(id)))) {
  ...
  if ((step as any)?.graveyardExileAbilityAsCost === true) {
    try {
      ...
      if (manaOnly) {
        const { getOrInitManaPool, validateAndConsumeManaCostFromPool } = await import('./util.js');
        const pool = getOrInitManaPool(game.state, controllerId) as any;
        const paid = validateAndConsumeManaCostFromPool(pool, manaOnly, { logPrefix: '[graveyardExileAbilityAsCost]' });
        if (!paid.ok) {
          emitToPlayer(io, controllerId, 'error', { code: 'INSUFFICIENT_MANA', message: (paid as any).error || 'Insufficient mana.' });
          return;
        }
