...
function getCastFromGraveyardActivationCost(card: any, abilityId: string, context?: { game?: any; playerId?: string }): { manaCost?: string; lifeCost?: number; hasAbility?: boolean } {
  ...
  const keywordPatterns: Record<string, RegExp> = {
    flashback: /flashback\s*[—-]?\s*(\{[^}]+\}(?:\s*\{[^}]+\})*)/i,
    'jump-start': /jump-start\s*[—-]?\s*(\{[^}]+\}(?:\s*\{[^}]+\})*)/i,
    escape: /escape\s*[—-]?\s*(\{[^}]+\}(?:\s*\{[^}]+\})*)/i,
    disturb: /disturb\s*[—-]?\s*(\{[^}]+\}(?:\s*\{[^}]+\})*)/i,
  };
...
type CastFromGraveyardExileCost = {
  exileCount: number;
  exileCountIsX?: boolean;
};

function getCastFromGraveyardExileCost(card: any, abilityId: string, context?: { game?: any; playerId?: string }): CastFromGraveyardExileCost | undefined {
  const normalizedAbilityId = String(abilityId || '').trim().toLowerCase();
  const oracleText = String(card?.oracle_text || '');

  if (normalizedAbilityId === 'flashback') {
    const flashbackLine = oracleText
      .split(/\r?\n/)
      .find((line) => /\bflashback\b/i.test(line));
    if (flashbackLine && /\bexile\s+x\s+cards?\s+from\s+your\s+graveyard\b/i.test(flashbackLine)) {
      return { exileCount: 0, exileCountIsX: true };
    }
  }
...
function getCastFromGraveyardSacrificeCost(
  ...
  const lines = String(card?.oracle_text || '').split(/\r?\n/);
  const keywordLine = lines
    .find((line) => line.toLowerCase().includes(normalizedAbilityId.replace('-', '')) || line.toLowerCase().includes(normalizedAbilityId));
  const additionalCostLine = lines.find((line) => /\bas an additional cost to cast this spell\b/i.test(line) && /\bsacrifice\b/i.test(line));
  const sacrificeLine = /\bsacrifice\b/i.test(String(keywordLine || '')) ? keywordLine : additionalCostLine;
  if (!sacrificeLine) return undefined;

  const sacrificeInfo = parseSacrificeCost(sacrificeLine);
...
function enforceSorcerySpeedGraveyardAbility(socket: any, game: any, playerId: string, abilityName: string): boolean {
  const phase = String((game.state as any).phase || '');
  ...
  if (activePlayer !== playerId) {
    socket.emit('error', {
      code: 'NOT_YOUR_TURN',
      message: `${abilityName} can only be activated on your turn (sorcery speed)`,
    });
    return false;
  }
  ...
  if (!isMainPhase || stack.length > 0) {
    socket.emit('error', {
      ...
      message: `${abilityName} can only be activated at sorcery speed (main phase, empty stack)`,
    });
    return false;
  }
...
function buildGraveyardCastSpellCard(card: any, abilityId: string): any {
  if (String(abilityId || '').trim().toLowerCase() === 'disturb') {
...
function getAuraEnchantTargeting(card: any): {
  auraTargetType: 'creature' | 'player' | 'opponent' | 'permanent' | 'artifact' | 'land' | 'enchantment';
  auraControllerRestriction: 'you_control' | 'opponent_controls' | 'any';
} | null {
  const typeLine = String(card?.type_line || '').toLowerCase();
  const oracleText = String(card?.oracle_text || '');
  if (!typeLine.includes('aura') || !/^enchant\s+/i.test(oracleText)) {
    return null;
  }
  ...
  const restriction = String(enchantMatch[2] || '').toLowerCase();
  ...
  return {
    auraTargetType: String(enchantMatch[1] || 'creature').toLowerCase() as 'creature' | 'player' | 'opponent' | 'permanent' | 'artifact' | 'land' | 'enchantment',
    auraControllerRestriction,
  };
}

function buildGraveyardAuraValidTargets(
  game: any,
  controllerId: string,
  auraTargetType: 'creature' | 'player' | 'opponent' | 'permanent' | 'artifact' | 'land' | 'enchantment',
  auraControllerRestriction: 'you_control' | 'opponent_controls' | 'any',
): Array<{ id: string; name: string; type: string; controller?: string; imageUrl?: string; life?: number; isOpponent?: boolean; typeLine?: string }> {
  const players = Array.isArray(game.state?.players) ? game.state.players : [];
  ...
  if (auraTargetType === 'opponent') {
    return players
      .filter((player: any) => player?.id && String(player.id) !== controllerId)
      .map((player: any) => ({
        id: String(player.id),
        name: player.name || String(player.id),
        type: 'player',
  ...
  return battlefield
    .filter((permanent: any) => {
      ...
      if (auraControllerRestriction === 'you_control' && String(permanent.controller || '') !== controllerId) return false;
      if (auraControllerRestriction === 'opponent_controls' && String(permanent.controller || '') === controllerId) return false;

      const typeLine = String(permanent.card?.type_line || '').toLowerCase();
      if (auraTargetType === 'permanent') return true;
      return typeLine.includes(auraTargetType);
    })
    .map((permanent: any) => ({
...
function buildGraveyardSpellCastValidTargets(
  ...
  const targetFilterExcludeTypes = Array.isArray((targetReqs as any)?.targetFilterExcludeTypes)
    ? ((targetReqs as any).targetFilterExcludeTypes as any[]).map((value: any) => String(value || '').toLowerCase()).filter(Boolean)
    : [];
      ...
      case 'noncreature permanent':
        validTargets.push(...battlefield
          .filter((permanent: any) => {
            ...
            const typeLine = String(permanent.card?.type_line || '').toLowerCase();
            if (targetFilterExcludeTypes.length > 0 && targetFilterExcludeTypes.some((typeWord) => typeLine.includes(typeWord))) {
              return false;
            }
            if (targetType === 'permanent') return true;
            if (targetType === 'nonland' || targetType === 'nonland permanent' || targetType === 'nonland_permanent') return !typeLine.includes('land');
            if (targetType === 'noncreature' || targetType === 'noncreature permanent') return !typeLine.includes('creature');
...
function queueGraveyardSpellCastTargetSelection(params: {
  ...
  const cardName = String(spellCard?.name || params.card?.name || 'Spell');
  const auraTargeting = getAuraEnchantTargeting(spellCard);

  let minTargets = 1;
  let maxTargets = 1;
  let targetDescription = 'target';
  let targetTypes: string[] = ['spell_target'];
  let validTargets: Array<{ id: string; name: string; type: string; controller?: string; zoneOwnerId?: string; imageUrl?: string; life?: number; isOpponent?: boolean; typeLine?: string }> = [];

  if (auraTargeting) {
    targetDescription = `Enchant ${auraTargeting.auraTargetType}`;
    targetTypes = ['aura_target'];
    validTargets = buildGraveyardAuraValidTargets(
      params.game,
      String(params.playerId),
      auraTargeting.auraTargetType,
      auraTargeting.auraControllerRestriction,
    );
  } else {
    const targetReqs = parseTargetRequirements(String(spellCard?.oracle_text || ''), {
    ...
    minTargets = Number(targetReqs.minTargets ?? 1);
    maxTargets = Number(targetReqs.maxTargets ?? minTargets);
    targetDescription = String(targetReqs.targetDescription || 'target');
    targetTypes = Array.isArray(targetReqs.targetTypes) ? targetReqs.targetTypes : ['spell_target'];
    validTargets = buildGraveyardSpellCastValidTargets(params.game, String(params.playerId), targetReqs);
  }
  ...
  if (!existing) {
    const targetStep = ResolutionQueueManager.addStep(params.gameId, {
      ...
      sourceName: cardName,
      sourceImage: spellCard?.image_uris?.small || spellCard?.image_uris?.normal || params.card?.image_uris?.small || params.card?.image_uris?.normal,
      ...
      sacrificedPermanentIdsForCost: Array.isArray(params.sacrificedPermanentIdsForCost) && params.sacrificedPermanentIdsForCost.length > 0
        ...
        : undefined,
      xValue: Number.isFinite(Number(params.xValue)) ? Math.max(0, Math.floor(Number(params.xValue))) : undefined,
    } as any);
...
export function finalizeGraveyardSpellCast(params: {
  ...
  const stackItem = {
    ...
    card: spellCard,
    ...(params.manaCost ? { manaCost: params.manaCost } : {}),
    castFromGraveyard: true,
    castSourceZone: 'graveyard',
    fromZone: 'graveyard',
    ...
    ...(Number.isFinite(Number(params.xValue)) ? { xValue: Math.max(0, Math.floor(Number(params.xValue))) } : {}),
  ...
  params.io.to(params.gameId).emit('chat', {
    ...
    gameId: params.gameId,
    from: 'system',
    message: `${getPlayerName(params.game, params.playerId)} cast ${String(spellCard?.name || card?.name || 'that spell')} using ${params.abilityId}.`,
    ts: Date.now(),
  });
...
export function continueCastFromGraveyardAbility(params: {
  ...
  const paidGraveyardExileCount = Array.isArray(params.exiledCardIdsFromGraveyardForCost) ? params.exiledCardIdsFromGraveyardForCost.length : 0;
  const hasPaidGraveyardExileCost = exileCost
    ? (exileCost.exileCountIsX === true
      ? Number.isFinite(Number(params.xValue))
      : paidGraveyardExileCount >= exileCost.exileCount)
    : true;
  ...
  if (exileCost && !hasPaidGraveyardExileCost) {
    ...
    if (!exileCost.exileCountIsX && eligibleGraveyardCards.length < exileCost.exileCount) {
      params.emitError({
        code: 'CANNOT_PAY_COST',
    ...
    const dynamicExileCost = exileCost.exileCountIsX === true;
    const minExileTargets = dynamicExileCost ? 0 : exileCost.exileCount;
    const maxExileTargets = dynamicExileCost ? eligibleGraveyardCards.length : exileCost.exileCount;
    const description = dynamicExileCost
      ? `${cardName}: Exile any number of other cards from your graveyard to set X and cast it using ${params.abilityId}.`
      : `${cardName}: Exile ${exileCost.exileCount} other card${exileCost.exileCount === 1 ? '' : 's'} from your graveyard to cast it using ${params.abilityId}.`;
    const title = dynamicExileCost
      ? `Choose cards to exile for X on ${cardName}`
      : `Exile ${exileCost.exileCount} other card${exileCost.exileCount === 1 ? '' : 's'} for ${cardName}`;
    ...
    const exileStep = ResolutionQueueManager.addStep(params.gameId, {
      ...
      minTargets: minExileTargets,
      maxTargets: maxExileTargets,
      destination: 'exile',
      cardId: params.cardId,
      cardName,
      title,
      ...
      exileCountIsX: dynamicExileCost || undefined,
      abilityId: params.abilityId,
      manaCost: recordedManaCost || undefined,
      lifeToPayForCost: recordedLifeCost > 0 ? recordedLifeCost : undefined,
      targetsForCast: normalizedTargets.length > 0 ? normalizedTargets : undefined,
  ...
  if (sacrificeCost && !(Array.isArray(params.sacrificedPermanentIdsForCost) && params.sacrificedPermanentIdsForCost.length >= sacrificeCost.sacrificeCount)) {
    ...
    } else if (sacrificeCost.sacrificeType === 'nonland_permanent') {
      sacrificeLabel = 'nonland permanent';
    }
    ...
    if (!existingSacrificeStep) {
      const sacrificeStep = ResolutionQueueManager.addStep(params.gameId, {
        ...
        sacrificePermanentType: sacrificeCost.sacrificeType === 'artifact_or_creature' ? 'permanent' : sacrificeCost.sacrificeType === 'nonland_permanent' ? 'nonland permanent' : sacrificeCost.sacrificeType,
...
export function registerInteractionHandlers(io: Server, socket: Socket) {
  ...
  socket.on(
    ...
    (payload?: { gameId?: unknown; cardId?: unknown; abilityId?: unknown; targets?: unknown; xValue?: unknown }) => {
      const gameIdRaw = payload?.gameId;
      const cardId = payload?.cardId;
      const abilityId = payload?.abilityId;
      const xValue = typeof payload?.xValue === 'number' && Number.isFinite(payload.xValue)
        ? Math.max(0, Math.floor(Number(payload.xValue)))
        : undefined;
    ...
    if (abilityId === "flashback" || abilityId === "jump-start" || abilityId === "retrace" || abilityId === "escape" || abilityId === "graveyard-cast") {
      ...
      if (!enforceSorcerySpeedGraveyardAbility(socket, game, String(pid), 'Unearth')) {
        return;
      }
      ...
      if (!enforceSorcerySpeedGraveyardAbility(socket, game, String(pid), abilityId === 'eternalize' ? 'Eternalize' : 'Embalm')) {
        return;
      }
      ...
      const requiresSorcerySpeed = /activate(?:\s+this ability)?\s+only\s+(?:as a sorcery|any time you could cast a sorcery)/i.test(String(card?.oracle_text || ''));

      if (requiresSorcerySpeed && !enforceSorcerySpeedGraveyardAbility(socket, game, String(pid), cardName)) {
        return;
      }
      ...
      if (!enforceSorcerySpeedGraveyardAbility(socket, game, String(pid), 'Scavenge')) {
        return;
      }
            ...
            const validTargets = battlefield
              .filter((permanent: any) => {
                ...
                return canPermanentBeTargetedByPlayer(permanent as any, game.state as any, pid as any);
              })
              .map((permanent: any) => ({
                id: String(permanent.id),
                ...
                type: 'permanent',
                controller: String(permanent.controller || ''),
                isOpponent: String(permanent.controller || '') !== String(pid),
                typeLine: permanent.card?.type_line,
              }));
            if (validTargets.length === 0) {
              socket.emit('error', {
                code: 'NO_VALID_TARGETS',
                message: `${cardName} requires a target creature for scavenge, but none are available.`,
              });
              return;
            }
      ...
      const targetStep = ResolutionQueueManager.addStep(gameId, {
        ...
        description: `Choose target creature for ${cardName}`,
        mandatory: true,
        validTargets,
      ...
      try {
        ...
        debugWarn(1, 'appendEvent(resolveTopOfStackPrompt:scavenge) failed:', e);
      }
      ...
      io.to(gameId).emit("chat", {
        ...
        gameId,
        from: "system",
        message: `${getPlayerName(game, pid)} scavenged ${cardName}. Choose target creature for ${power} +1/+1 counter${power === 1 ? '' : 's'}.`,
        ts: Date.now(),
      });
    } else if (abilityId === "encore") {
      if (!enforceSorcerySpeedGraveyardAbility(socket, game, String(pid), 'Encore')) {
        return;
      }
  ...
  socket.on(
    ...
    let sacrificeType: 'creature' | 'artifact' | 'enchantment' | 'land' | 'permanent' | 'nonland_permanent' | 'self' | 'artifact_or_creature' | null = null;
    ...
    if (sacrificeType && sacrificeType !== 'self') {
      ...
      if (costLower.includes('discard')) {
        ...
        if (discardMatch || discardTypedMatch) {
          ...
          } else if (sacrificeType === 'nonland_permanent') {
            sacrificeLabel = 'nonland permanent';
            sacrificeLabelPlural = 'nonland permanents';
          }
    ...
    if (!isManaAbility) {
      ...
      if (sacrificeType === 'self') {
        ...
        try {
          const battlefield = Array.isArray(game.state?.battlefield) ? game.state.battlefield : [];
          const sacrificedSourcePermanent = battlefield.find((entry: any) => entry && String((entry as any).id || '') === String(permanentId)) as any;
          const tokenGraveyardSnapshot = sacrificedSourcePermanent?.isToken && sacrificedSourcePermanent?.card
            ? { ...(sacrificedSourcePermanent.card || {}), zone: 'graveyard' }
            : null;
          const graveyardOwnerId = String(sacrificedSourcePermanent?.owner || sacrificedSourcePermanent?.controller || pid || '').trim();
          const ctx = {
            state: game.state,
            libraries: (game as any).libraries,
            bumpSeq: typeof (game as any).bumpSeq === 'function' ? (game as any).bumpSeq.bind(game) : (() => {}),
      ...
      const hasParsedGraveyardTargets = parsedGraveyardTargetReqs.needsTargets
        ...
        && parsedGraveyardTargetReqs.targetTypes.every((targetType) => String(targetType || '').toLowerCase().startsWith('graveyard_'));

      if (hasParsedGraveyardTargets) {
        const players = Array.isArray(game.state?.players) ? game.state.players : [];
        ...
        if (validTargets.length < parsedGraveyardTargetReqs.minTargets) {
          socket.emit('error', {
            code: 'NO_VALID_TARGETS',
            message: `${cardName}'s ability requires at least ${parsedGraveyardTargetReqs.minTargets} graveyard target(s) but only ${validTargets.length} are available.`,
          });
          return;
        }
