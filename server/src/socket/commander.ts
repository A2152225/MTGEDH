...
export function registerCommanderHandlers(io: Server, socket: Socket) {
  ...
  socket.on("dumpLibrary", ({ gameId }: { gameId: string }) => {
    try {
      ...
      socket.emit("debugLibraryDump", { gameId: (payload as any)?.gameId, playerId: socket.data.playerId, library: null, error: String(err) });
    }
  });
  ...
  socket.on("dumpImportedDeckBuffer", ({ gameId }: { gameId: string }) => {
    try {
      ...
      socket.emit("debugImportedDeckBuffer", { gameId: (payload as any)?.gameId, playerId: socket.data.playerId, buffer: null, error: String(err) });
    }
  });
  ...
  socket.on("dumpCommanderState", ({ gameId }: { gameId: string }) => {
    try {
      ...
      socket.emit("debugCommanderState", { gameId: (payload as any)?.gameId, playerId: socket.data.playerId, state: null, error: String(err) });
    }
  });
}