...
describe('OPENING_HAND_ACTIONS validate-before-complete (integration)', () => {
  const gameId = 'test_opening_hand_actions_validate_before_complete';
  ...
  it('does not consume the step on invalid selection', async () => {
    ...
    const leyline = {
      id: 'l1',
      name: 'Leyline of Something',
      oracle_text: 'If Leyline of Something is in your opening hand, you may begin the game with it on the battlefield.',
    ...
    const step = queue.steps.find((s: any) => s.type === 'opening_hand_actions');
    expect(step).toBeDefined();

    const stepId = String((step as any).id);

    await handlers['submitResolutionResponse']({ gameId, stepId, selections: ['n1'] });
    ...
    await handlers['submitResolutionResponse']({ gameId, stepId, selections: ['l1'] });
