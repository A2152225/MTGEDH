...
describe('VOTE validate-before-complete (integration)', () => {
  const gameId = 'test_vote_validate_before_complete';
  ...
  it('does not consume the step on invalid selection', async () => {
    ...
    const step = queue.steps.find((s: any) => s.type === 'vote');
    ...
    await handlers['submitResolutionResponse']({ gameId, stepId, selections: { choice: 'a', voteCount: 1 } });
