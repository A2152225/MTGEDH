...
describe('undo broadcast automation suppression', () => {
  ...
  it('holds human auto-pass after undo until that player takes an action', () => {
    const gameId = 'undo_broadcast_pause_until_action';
    const ctx = createContext(gameId);
    ...
    pauseHumanAutoPassUntilAction(game, 'p1');

    broadcastGame(io, game, gameId);
    broadcastGame(io, game, gameId);

    expect(passPriority).not.toHaveBeenCalled();
    expect((game.state as any)._pauseHumanAutoPassUntilActionFor).toBe('p1');

    expect(clearHumanAutoPassPauseOnAction(game, 'p2', 'other player action')).toBe(false);
    expect((game.state as any)._pauseHumanAutoPassUntilActionFor).toBe('p1');

    expect(clearHumanAutoPassPauseOnAction(game, 'p1', 'manual action')).toBe(true);
    expect((game.state as any)._pauseHumanAutoPassUntilActionFor).toBeUndefined();
