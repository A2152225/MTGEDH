...
describe('COUNTER_TARGET validate-before-complete (integration)', () => {
  const gameId = 'test_counter_target_validate_before_complete';
  ...
  it('does not consume the step on invalid target selection', async () => {
    ...
    (game.state as any).battlefield = [
      {
        ...
        owner: p1,
        counters: {},
        card: { id: 'c1', name: 'Test Permanent', type_line: 'Artifact — Test' },
      },
    ];
    ...
    ResolutionQueueManager.addStep(gameId, {
      ...
      playerId: p1 as any,
      description: 'Put a counter',
      mandatory: true,
      counterType: 'charge',
      targetController: 'you',
      sourceName: 'Counter Ability',
    } as any);
    ...
    const step = queue.steps.find((s: any) => s.type === 'counter_target');
    ...
    const perm = bf.find(p => p.id === targetId);
    expect(Number(perm?.counters?.charge || 0)).toBe(1);
  });
});
