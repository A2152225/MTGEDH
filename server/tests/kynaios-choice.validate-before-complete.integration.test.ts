...
describe('KYNAIOS_CHOICE validate-before-complete (integration)', () => {
  const gameId = 'test_kynaios_choice_validate_before_complete';
  ...
  it('does not consume the step on invalid selection', async () => {
    ...
    const step = queue.steps.find((s: any) => s.type === 'kynaios_choice');
    expect(step).toBeDefined();

    const stepId = String((step as any).id);

    await handlers['submitResolutionResponse']({ gameId, stepId, selections: { choice: 'play_land' } });
    ...
    await handlers['submitResolutionResponse']({ gameId, stepId, selections: { choice: 'decline' } });
