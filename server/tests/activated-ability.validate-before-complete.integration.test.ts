...
describe('Activated ability validate-before-complete (integration)', () => {
  ...
  it('ACTIVATED_ABILITY does not consume the step on invalid response payload, then consumes once valid', async () => {
    ...
    ResolutionQueueManager.addStep(gameId, {
      ...
      playerId: p1 as any,
      description: 'Resolve activated ability',
      mandatory: true,
      sourceId: 'perm_missing',
      sourceName: 'Test Permanent',
      permanentId: 'perm_missing',
      permanentName: 'Test Permanent',
      abilityType: 'generic',
    ...
    const step = queue.steps.find((s: any) => s.type === 'activated_ability');
    expect(step).toBeDefined();
    const stepId = String((step as any).id);

    await handlers['submitResolutionResponse']({ gameId, stepId, selections: ['unexpected'] });
