...
describe('BOUNCE_LAND_CHOICE validate-before-complete (integration)', () => {
  const gameId = 'test_bounce_land_choice_validate_before_complete';
  ...
  it('does not consume the step on invalid selection', async () => {
    ...
    const step = queue.steps.find((s: any) => s.type === 'bounce_land_choice');
