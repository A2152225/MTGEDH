...
describe('PLAYER_CHOICE validate-before-complete (integration)', () => {
  const gameId = 'test_player_choice_validate_before_complete';
  ...
  it('does not consume the step on invalid selection', async () => {
    ...
    ResolutionQueueManager.addStep(gameId, {
      ...
      mandatory: true,
      players: [
        { id: p1, name: 'P1', isSelf: true },
        { id: p2, name: 'P2', isOpponent: true },
      ],
      opponentOnly: true,
    } as any);
    ...
    const step = queue.steps.find((s: any) => s.type === 'player_choice');
    expect(step).toBeDefined();
    const stepId = String((step as any).id);

    await handlers['submitResolutionResponse']({ gameId, stepId, selections: p1 });
