...
describe('castSpellFromHand in-room authorization (integration)', () => {
  const gameId = 'test_cast_spell_from_hand_inroom';
  ...
  it('rejects castSpellFromHand when socket is not in the game room', async () => {
    const p1 = 'p1';
    ...
    await handlers['castSpellFromHand']({ gameId, cardId: 'spell1', targets: [] });

    const err = emitted.find(e => e.event === 'error');
    expect(err?.payload?.code).toBe('NOT_IN_GAME');
  });

  it('rejects castSpellFromHand when socket.data.gameId mismatches (even if in room)', async () => {
    const p1 = 'p1';
