...
describe('CLASH validate-before-complete (integration)', () => {
  const gameId = 'test_clash_validate_before_complete';
  ...
  it('does not consume the step on invalid selection', async () => {
    ...
    (game as any).libraries.set(p1, [{ id: 'c_top', name: 'Top Card', zone: 'library' }]);
    ...
    ResolutionQueueManager.addStep(gameId, {
      ...
      playerId: p1 as any,
      description: 'Clash now',
      mandatory: true,
      revealedCard: { id: 'c_top', name: 'Top Card' },
      opponentId: 'p2',
    } as any);
    ...
    const step = queue.steps.find((s: any) => s.type === 'clash');
    ...
    await handlers['submitResolutionResponse']({ gameId, stepId, selections: { putOnBottom: true } });
