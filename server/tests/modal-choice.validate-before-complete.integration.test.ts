...
describe('MODAL_CHOICE validate-before-complete (integration)', () => {
  const gameId = 'test_modal_choice_validate_before_complete';
  ...
  it('does not consume the step on invalid selection for put-from-hand modal', async () => {
    ...
    const step = queue.steps.find((s: any) => s.type === 'modal_choice');
    expect(step).toBeDefined();

    const stepId = String((step as any).id);

    await handlers['submitResolutionResponse']({ gameId, stepId, selections: 'c2' });
