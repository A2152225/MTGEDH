...
describe('CREATURE_TYPE_CHOICE validate-before-complete (integration)', () => {
  const gameId = 'test_creature_type_choice_validate_before_complete';
  ...
  it('does not consume the step on invalid selection', async () => {
    ...
    const step = queue.steps.find((s: any) => s.type === 'creature_type_choice');
    ...
    await handlers['submitResolutionResponse']({ gameId, stepId, selections: 'Goblin' });
