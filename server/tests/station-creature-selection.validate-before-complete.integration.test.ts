...
describe('STATION_CREATURE_SELECTION validate-before-complete (integration)', () => {
  const gameId = 'test_station_creature_selection_validate_before_complete';
  ...
  it('does not consume the step on invalid selection', async () => {
    ...
    (game.state as any).battlefield = [
      {
        ...
        tapped: false,
        counters: { charge: 0 },
        card: { id: 'c_station', name: 'Station', type_line: 'Artifact — Station' },
      },
      {
        id: creatureId,
        controller: p1,
        ...
        basePower: 2,
        baseToughness: 2,
        card: { id: 'c_cre', name: 'Crewmate', type_line: 'Creature — Test', power: '2', toughness: '2' },
      },
    ];
    ...
    ResolutionQueueManager.addStep(gameId, {
      ...
      playerId: p1 as any,
      description: 'Tap a creature to station',
      mandatory: true,
      sourceId: stationId,
      sourceName: 'Station',
    ...
    const step = queue.steps.find((s: any) => s.type === 'station_creature_selection');
    ...
    const station = bf.find(p => p.id === stationId);
    const creature = bf.find(p => p.id === creatureId);
    expect(Boolean(creature?.tapped)).toBe(true);
    expect(Number(station?.counters?.charge || 0)).toBeGreaterThan(0);
  });
});
