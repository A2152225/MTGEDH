...
describe('FATESEAL validate-before-complete (integration)', () => {
  const gameId = 'test_fateseal_validate_before_complete';
  ...
  it('does not consume the step on invalid selection', async () => {
    ...
    const rest = [{ id: 'c3', name: 'Card 3', zone: 'library' }];
    (game as any).libraries?.set?.(p2, [...topCards, ...rest]);

    (game.state as any).zones = {
      [p1]: { graveyard: [], graveyardCount: 0, exile: [], exileCount: 0, hand: [], handCount: 0, libraryCount: 0 },
      [p2]: { graveyard: [], graveyardCount: 0, exile: [], exileCount: 0, hand: [], handCount: 0, libraryCount: topCards.length + rest.length },
    };
    (game.state as any).battlefield = [];

    const stepCards = topCards.map(c => ({ id: c.id, name: c.name }));
    ...
    const step = queue.steps.find((s: any) => s.type === 'fateseal');
    ...
    await handlers['submitResolutionResponse']({
      gameId,
      stepId,
      selections: { keepTopOrder: ['c2'], bottomOrder: ['c1'] },
    });
