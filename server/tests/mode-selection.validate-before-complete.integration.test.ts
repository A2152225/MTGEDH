...
describe('MODE_SELECTION validate-before-complete (integration)', () => {
  const gameId = 'test_mode_selection_validate_before_complete';
  ...
  it('does not consume the step on invalid selection', async () => {
    ...
    const step = queue.steps.find((s: any) => s.type === 'mode_selection');
    expect(step).toBeDefined();

    const stepId = String((step as any).id);

    await handlers['submitResolutionResponse']({ gameId, stepId, selections: 'c' });
    ...
    await handlers['submitResolutionResponse']({ gameId, stepId, selections: 'a' });
