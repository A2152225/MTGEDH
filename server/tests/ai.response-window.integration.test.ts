...
describe('AI response-window integration', () => {
  ...
  it('still attempts a cast when the only workable response path is a non-tap return-to-hand mana activation first', async () => {
    const gameId = createTestGameId('hand_counterspell_return_to_hand_payment');
    createGameIfNotExists(gameId, 'commander', 40);
    const game = ensureGame(gameId);
    if (!game) throw new Error('ensureGame returned undefined');
