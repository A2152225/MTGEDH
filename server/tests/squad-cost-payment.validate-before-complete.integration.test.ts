...
describe('SQUAD_COST_PAYMENT validate-before-complete (integration)', () => {
  const gameId = 'test_squad_cost_payment_validate_before_complete';
  ...
  it('does not consume the step on invalid selection', async () => {
    ...
    (game.state as any).zones = {
      [p1]: {
        hand: [{ id: 'spell1', name: 'Squad Spell', oracle_text: '' }],
        handCount: 1,
      },
    };
    ...
    const step = queue.steps.find((s: any) => s.type === 'squad_cost_payment');
