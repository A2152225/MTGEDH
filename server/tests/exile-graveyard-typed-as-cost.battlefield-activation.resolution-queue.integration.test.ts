...
describe('Exile-from-graveyard-typed-as-activation-cost via Resolution Queue (integration)', () => {
  const gameId = 'test_exile_graveyard_typed_activation_cost_resolution_queue';
  ...
  it('filters GRAVEYARD_SELECTION to eligible type and resumes activation after exiling', async () => {
    ...
    (game.state as any).battlefield = [
      {
        ...
        card: {
          name: 'Test Relic',
          type_line: 'Artifact',
          oracle_text: 'Exile a creature card from your graveyard: Draw a card.',
    ...
    (game.state as any).zones = {
      [p1]: {
        hand: [],
        graveyard: [
          { id: 'g_1', name: 'Dead Creature', type_line: 'Creature', zone: 'graveyard' },
          { id: 'g_2', name: 'Dead Spell', type_line: 'Instant', zone: 'graveyard' },
        ],
        exile: [],
        handCount: 0,
        graveyardCount: 2,
    ...
    expect(step.validTargets.length).toBe(1);
    expect(String(step.validTargets[0]?.id)).toBe('g_1');
