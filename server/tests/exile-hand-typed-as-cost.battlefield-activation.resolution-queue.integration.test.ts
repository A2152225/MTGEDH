...
describe('Exile-from-hand-typed-as-activation-cost via Resolution Queue (integration)', () => {
  const gameId = 'test_exile_hand_typed_activation_cost_resolution_queue';
  ...
  it('filters DISCARD_SELECTION(hand) to eligible type and resumes activation after exiling', async () => {
    ...
    (game.state as any).battlefield = [
      {
        ...
        card: {
          name: 'Test Relic',
          type_line: 'Artifact',
          oracle_text: 'Exile an instant card from your hand: Draw a card.',
    ...
    (game.state as any).zones = {
      [p1]: {
        hand: [
          { id: 'h_1', name: 'Hand Creature', type_line: 'Creature', zone: 'hand' },
          { id: 'h_2', name: 'Hand Instant', type_line: 'Instant', zone: 'hand' },
        ],
        graveyard: [],
        exile: [],
        handCount: 2,
    ...
    expect(Array.isArray(step.hand)).toBe(true);
    expect(step.hand.length).toBe(1);
    expect(String(step.hand[0]?.id)).toBe('h_2');
    ...
    expect((zones.hand as any[]).some((c: any) => c && c.id === 'h_2')).toBe(false);
    expect((zones.exile as any[]).some((c: any) => c && c.id === 'h_2')).toBe(true);
