...
describe('BOTTOM_ORDER validate-before-complete (integration)', () => {
  const gameId = 'test_bottom_order_validate_before_complete';
  ...
  it('does not consume the step on invalid selection', async () => {
    ...
    ResolutionQueueManager.addStep(gameId, {
      ...
      playerId: p1 as any,
      description: 'Put these on bottom',
      mandatory: true,
      cards,
    } as any);
    ...
    const step = queue.steps.find((s: any) => s.type === 'bottom_order');
    expect(step).toBeDefined();

    const stepId = String((step as any).id);

    await handlers['submitResolutionResponse']({ gameId, stepId, selections: { bottomOrder: ['c1', 'c1', 'c2'] } });
    ...
    await handlers['submitResolutionResponse']({ gameId, stepId, selections: { bottomOrder: ['c2', 'c3', 'c1'] } });
