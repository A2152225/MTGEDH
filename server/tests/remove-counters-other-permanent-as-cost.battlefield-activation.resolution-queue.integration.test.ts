...
describe('Remove-counters-from-other-permanent as activation cost via Resolution Queue (integration)', () => {
  const gameId = 'test_remove_counters_other_perm_activation_cost_resolution_queue';
  ...
  it('enqueues TARGET_SELECTION and resumes activation after removing counters from an artifact you control', async () => {
    ...
    (game.state as any).battlefield = [
      {
        ...
        card: {
          name: 'Test Engine',
          type_line: 'Artifact',
          oracle_text: 'Remove a charge counter from an artifact you control: Draw a card.',
      ...
      },
      {
        ...
        card: {
          ...
          type_line: 'Artifact',
          oracle_text: '',
          image_uris: { small: 'https://example.com/battery.jpg' },
        },
      },
    ];
    ...
    expect((step.validTargets || []).some((t: any) => String(t?.id) === 'a_1')).toBe(true);
    ...
    // Step should be completed/removed from queue.
    const afterQueue = ResolutionQueueManager.getQueue(gameId);
    expect(afterQueue.steps.length).toBe(0);
    expect(afterQueue.completedSteps.length).toBeGreaterThan(0);
    const completed = afterQueue.completedSteps[afterQueue.completedSteps.length - 1] as any;
    expect(completed.type).toBe('target_selection');
    expect(completed.removeCountersAbilityAsCost).toBe(true);
    expect(Array.isArray(completed.validTargets)).toBe(true);
    expect((completed.validTargets || []).some((t: any) => String(t?.id) === 'a_1')).toBe(true);

    const socketErrors = emitted.filter((e) => e.event === 'error');
    expect(socketErrors).toEqual([]);

    const stackAfter = (game.state as any).stack || [];
    expect(stackAfter.length).toBe(1);

    const battery = (game.state as any).battlefield.find((p: any) => p.id === 'a_1');
    expect(battery).toBeDefined();
    expect(Number(battery.counters?.charge || 0)).toBe(1);
