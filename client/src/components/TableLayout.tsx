// Only the additions/changes around the Decks button and passing anchorEl are new; rest is unchanged.
...
// (All existing helper functions: clamp, sidePlan, buildPositions, computeExtents) ... unchanged for brevity.
// Full file retained from your current version with minimal edits.
...
function buildPositions(opts:{ total:number; boardW:number; boardH:number; seatGapX:number; seatGapY:number; centerClearX:number; centerClearY:number; sidePad:number; sideOrder:Side[] }): Array<{x:number;y:number;rotateDeg:number;side:Side}> {
      ...
      {enablePanZoom && (
        <div style={{
          ...
          <button type="button" onClick={()=>setCam(prev=>({ ...prev, z:clamp(prev.z/1.15,0.15,2.5) }))}>−</button>
              <button type="button" onClick={()=>setCam({x:0,y:0,z:1})}>Reset</button>
