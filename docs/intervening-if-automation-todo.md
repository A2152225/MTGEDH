...
- [x] That-player spell log determinism: "if that player didn't cast a spell this turn" / "if that player didn't cast a creature spell this turn" now use the same per-turn spell log helpers as the "you didn't cast ..." templates, avoiding unnecessary `null` when `spellsCastThisTurn` is absent; creature-spell checks remain conservative (`null`) when spell type lines are missing.
- [x] No-other-attackers completeness: "if no other creatures are attacking that player" now prefers `state.attackersDeclaredThisCombatByPlayer` when available so it can safely evaluate all declared attackers (and return deterministic `true/false`) instead of relying on best-effort battlefield scans.
- [x] Players-being-attacked poison check: "if one or more players being attacked are poisoned" now prefers `state.attackedDefendingPlayersThisCombatByPlayer` (defending-player tracking, including planeswalker/battle attacks) and uses a conservative battlefield fallback when the tracker is absent.
- [x] Deterministic additional-cost payment: Casting now persists replay-stable `additionalCostPaidKnown`/`additionalCostPaid` onto the triggering stack item so the intervening-if clause "if its additional cost was paid" can return deterministic `true/false` when we know the spell had an additional cost; without the known flag it remains positive-only.
- [x] Deterministic Warrior damage source refs: Damage-to-player triggers now persist replay-stable damage-source refs (`damageSourcePermanentIds`) and a computed boolean (`damageIncludedWarriorSource`) onto the queued triggered ability so the intervening-if clause "if any of that damage was dealt by a Warrior" can be evaluated deterministically at both trigger time and resolution time.
