...
- [x] Latest known broad validation was green after durable usage-limit counter consumption and per-turn reset: `git diff --check`, `npm test` (179 files / 6266 tests), `npm run build`, focused durable permission tests, and `npm run typecheck --workspace=server` passed.
...
- [ ] Integrate permissions with legal action generation.
  ...
  - Durable exact-card playable-from-exile permissions now feed the shared exile reader, spell candidate generation, land candidate generation, and live `requestCastSpell` / `playLand` exile permission checks. Central Oracle IR impulse exile and library-search exile grants now mirror their legacy `playableFromExile` markers into durable records, cleanup removes durable entries when the card leaves exile, and durable exile timing grants now propagate response-speed casting through both shared candidates and live request-cast timing checks. Guardrails: `server/tests/durable-permissions.test.ts`, `server/tests/can-respond.test.ts`, `server/tests/playable-from-exile.lifecycle.test.ts`, `server/tests/playable-from-exile.timing.integration.test.ts`.
  - Durable top-library play/cast permissions now feed the existing `canPlayLandsFromTop` / `canCastFromTop` readers, so shared land/spell candidates and live top-library `playLand` / `requestCastSpell` validation can consume durable-only library permissions. Durable library `without_paying_mana_cost` grants now flow through shared spell candidates and live top-library cast payment setup as `{0}` free-cast prompts, durable library flexible-mana metadata now lets shared/live top-library casts satisfy colored costs with any mana type, and durable library timing overrides now enforce `duringYourTurnOnly` / `sorcerySpeedOnly` windows while still treating `ignoreTiming` as response-speed authorization. Guardrails: `server/tests/durable-permissions.test.ts`, `server/tests/can-respond.test.ts`, `server/tests/top-library.cast-timing.integration.test.ts`.
  - Durable command-zone permission metadata now augments existing command-zone commander candidates without broadening who can cast from command zone: durable records can grant flash/ignore-timing access, tax-aware `without_paying_mana_cost` handling, and flexible mana spending to commanders already in the command zone. The same slice fixed command-zone payment prompts/completion so shared static command-zone adjustments are not double-applied after target selection or no-target payment setup. Guardrails: `server/tests/durable-permissions.test.ts`, `server/tests/can-respond.test.ts`, `server/tests/commander.shared-surface.integration.test.ts`.
...
- [ ] Support permission expiration.
  ...
  - Generic durable permission activation now filters records whose explicit usage counters are exhausted (`once`, `once_per_turn`, `one_shot`, and `maxUses`), and durable land/exile/library/command builders preserve `usageLimit` metadata for shared readers. Replay-safe counter writers now capture matched durable permissions before source-zone mutation and increment `used` / `usedThisTurn` only after successful persisted top-library casts, top-library land plays, and command-zone commander casts; `nextTurn` resets durable `usedThisTurn` counters so `once_per_turn` permissions become reusable without clearing lifetime `used`. Guardrails: `server/tests/durable-permissions.test.ts`, `server/tests/can-respond.test.ts`.
