...
function shouldIgnoreGap(raw: string, gapType: GapRecord['gapType']): boolean {
  ...
  if (/^as an additional cost to cast this spell, sacrifice a creature or$/.test(lower)) return true;
  if (/^claim the prize!?$/.test(lower)) return true;
  if (/^this ability triggers only once each turn/.test(lower)) return true;
  if (/^this effect reduces only the amount of colored mana you pay$/.test(lower)) return true;
  if (/^this mana can't be spent to cast a nonartifact spell\."?\)?$/.test(lower)) return true;
  if (/^tapped unless .+$/.test(lower)) return true;
  if (/^this vehicle becomes an artifact creature$/.test(lower)) return true;
  if (/^you may pay \{[^}]+\} and return a basic land you control to its owner's hand rather than pay this spell's mana cost$/.test(lower)) return true;
  if (/^choose (?:a )?player$/.test(lower)) return true;
  if (/^choose left or right$/.test(lower)) return true;
  if (/^an opponent chooses? (?:one|two) of (?:them|those cards)$/.test(lower)) return true;
  if (/^equip only as a sorcery\.\)?$/.test(lower)) return true;
  if (/^fading \d+\s*\(this creature enters with \w+ fade counters on it$/.test(lower)) return true;
  if (/^freerunning \{[^}]+\}(?:\{[^}]+\})*\s*\(you may cast this spell for its freerunning cost if you dealt combat damage to a player this turn with an assassin or commander\.\)?$/.test(lower)) return true;
  if (/^reveal this card as you draft it and note how many cards you've drafted this draft round, including this card$/.test(lower)) return true;
  if (/^that many$/.test(lower)) return true;
  if (/^until they exile (?:a|an) (?:nonland|instant or sorcery) card$/.test(lower)) return true;
  if (/^the high bidder$/.test(lower)) return true;
  if (/^this card's mana value is \d+\.?["')]*$/.test(lower)) return true;
  if (/^then shuffle\.?(?:"\)?|"\)|\))?$/.test(lower)) return true;
  if (/^damage dealt by it also causes you to gain that much life\.?\)*$/.test(lower)) return true;
  if (/^each artifact you tap after you're done activating mana abilities pays for \{1\}\.\)*$/.test(lower)) return true;
  if (/^each copy targets a different one of those creatures$/.test(lower)) return true;
  if (/^enchanted creature has this permanent$/.test(lower)) return true;
  if (/^enchanted creature is 1\/1\.\)*$/.test(lower)) return true;
  if (/^entwine (?:\{[^}]+\})+ \(choose both if you pay the entwine cost\.\)$/.test(lower)) return true;
  if (/^under the control of an opponent of your choice$/.test(lower)) return true;
  if (/^when the last is removed, (?:sacrifice it|you may cast it without paying its mana cost)\.?(?:\))?$/.test(lower)) return true;
  if (/^when you pay this cost one or more times, put that many \+1\/\+1 counters on this creature$/.test(lower)) return true;
  if (/^then shuffle and put those cards on top in any order$/.test(lower)) return true;
  if (/^then shuffles the rest into their library$/.test(lower)) return true;
  if (/^then you may pay one or more \{e\}$/.test(lower)) return true;
  if (/^you may (?:blight \d+|forage)$/.test(lower)) return true;
  if (/^you may reveal this card from your opening hand$/.test(lower)) return true;
  if (/^a creature with indestructible can't be destroyed by damage\.?(?:\))?$/.test(lower)) return true;
  if (/^activate each exhaust ability only once\.?(?:\))?$/.test(lower)) return true;
  if (/^amplify \d+\s*\(as this creature enters, put .+ counters? on it for each .+ card you reveal in your hand\.?(?:\))?$/.test(lower)) return true;
  if (/^backup \d+\s*\(when this creature enters, put .+ counters? on target creature$/.test(lower)) return true;
  if (/^an opponent chooses one -$/.test(lower)) return true;
  if (/^(?:another|any)$/.test(lower)) return true;
  if (/^any player may activate this ability but only as a sorcery$/.test(lower)) return true;
  if (/^at the beginning of each of your upkeeps, copy this spell except for its epic ability$/.test(lower)) return true;
  if (/^at the beginning of your upkeep, remove a time counter$/.test(lower)) return true;
  if (/^\{s\} can be paid with one mana from a snow source\.?(?:\))?$/.test(lower)) return true;
  if (/^\{tk\}(?:\{tk\})*\s+-\s+\d+\/\d+$/.test(lower)) return true;
  if (/^each opponent attacking that player does the same$/.test(lower)) return true;
  if (/^if it's a creature, it has haste\.\)?$/.test(lower)) return true;
  if (/^if that's another creature, it gains the following abilities until end of turn\.\)?$/.test(lower)) return true;
  if (/^if you do, it becomes plotted$/.test(lower)) return true;
  if (/^join forces\s+-\s+starting with you, each player may pay any amount of mana$/.test(lower)) return true;
  if (/^target opponent chooses a creature they control$/.test(lower)) return true;
  if (/^ripple \d+\s*\(when you cast this spell, you may reveal the top \w+ cards? of your library$/.test(lower)) return true;
  if (/^spell commander\s*\(this card can be your commander$/.test(lower)) return true;
  if (/^you can cast it on a commander in your command zone\.\)?$/.test(lower)) return true;
  if (/^tribute \d+\s*\(as this creature enters, an opponent of your choice may put \w+ \+1\/\+1 counters on it\.\)?$/.test(lower)) return true;
  if (/^undaunted\s*\(this spell costs \{1\} less to cast for each opponent\.\)?$/.test(lower)) return true;
  if (gapType === 'unknown-step') {
    if (/^(damage metadata|token creation metadata)$/.test(lower)) return true;
    ...
    if (/^turn it face up any time for (?:its morph cost|its mana cost if it's a creature card)\.\)?$/.test(lower)) return true;
    if (/^then do it again\.\)?$/.test(lower)) return true;
    if (/^this vehicle becomes an artifact creature until end of turn$/.test(lower)) return true;
    if (/^as an additional cost to cast this spell, sacrifice a creature or$/.test(lower)) return true;
    if (/^at the beginning of your end step, remove a time counter from it\.\)?$/.test(lower)) return true;
    if (/^claim the prize!?$/.test(lower)) return true;
    if (/^commander enchantment\s*\(this aura enchants a commander creature, and remains attached to the creature as it moves between any face-up zones$/.test(lower)) return true;
    if (/^epic\s*\(for the rest of the game, you can't cast spells$/.test(lower)) return true;
    if (/^if any creatures with banding a player controls are blocking or being blocked by a creature, that player divides that creature's combat damage, not its controller, among any of the creatures it's being blocked by or is blocking\.\)?$/.test(lower)) return true;
    if (/^if chaos gets more votes or the vote is tied, chaos ensues$/.test(lower)) return true;
    if (/^if you do, add this card's effects to that spell\.\)?$/.test(lower)) return true;
    if (/^in limited, it can partner like other monocolored legends\.\)?$/.test(lower)) return true;
    if (/^it's an enchantment$/.test(lower)) return true;
