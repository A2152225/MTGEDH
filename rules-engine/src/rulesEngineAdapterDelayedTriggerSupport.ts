...
function isPermanentCardInAnyExile(state: GameState, permanentId: string): boolean {
  const targetId = String(permanentId || '').trim();
  if (!targetId) return false;

  for (const player of state.players || []) {
    const exile = Array.isArray((player as any)?.exile) ? (player as any).exile : [];
    if (exile.some((card: any) => String(card?.id || '').trim() === targetId)) {
      return true;
    }
  }
...
export function processDiesOrExileDelayedTriggersForState(args: {
  gameId: string;
  previousState: GameState;
  nextState: GameState;
  getStack: (gameId: string) => Stack | undefined;
  ...
  const nextRegistry = ((nextState as any).delayedTriggerRegistry || createDelayedTriggerRegistry()) as ReturnType<typeof createDelayedTriggerRegistry>;
  const watchedTriggers = previousRegistry.triggers.filter(
    trigger => trigger.timing === DelayedTriggerTiming.WHEN_DIES_OR_EXILED && String(trigger.watchingPermanentId || '').trim().length > 0
  );
  ...
  for (const previousPermanent of previousBattlefield) {
    ...
    const died = isPermanentCardInAnyGraveyard(nextState, permanentId);
    const wasExiled = isPermanentCardInAnyExile(nextState, permanentId);
    if (!died && !wasExiled) {
      const remainingTriggers = workingRegistry.triggers.filter(trigger => {
        if (trigger.timing !== DelayedTriggerTiming.WHEN_DIES_OR_EXILED) return true;
        return String(trigger.watchingPermanentId || '').trim() !== permanentId;
      });
      expiredTriggerCount += workingRegistry.triggers.length - remainingTriggers.length;
      workingRegistry = {
        ...workingRegistry,
  ...
  if (firedTriggers.length === 0) {
    ...
    return {
      ...
      log: expiredTriggerCount > 0 ? [`Expired ${expiredTriggerCount} watched dies-or-exile trigger(s)`] : [],
    };
  }
  ...
  return {
    ...
    log: firedTriggers.map(trigger => `Delayed dies-or-exile trigger fires: ${trigger.sourceName}`),
  };
}
