...
export function buildOracleIRExecutionContext(
  ...
  const hintChosenBasicLandType =
    typeof hint?.chosenBasicLandType === 'string' ? hint.chosenBasicLandType.trim() || undefined : undefined;
  ...
  const hintUnlessPaysManaChoice =
    hint?.unlessPaysManaChoice === 'pay' || hint?.unlessPaysManaChoice === 'decline'
      ...
      : undefined;
  const hintSpellCastCountThisTurn = Number(hint?.spellCastCountThisTurn);
  const hintNoncreatureSpellCastCountThisTurn = Number(hint?.noncreatureSpellCastCountThisTurn);
  ...
  const baseChosenBasicLandType =
    typeof baseSel?.chosenBasicLandType === 'string' ? baseSel.chosenBasicLandType.trim() || undefined : undefined;
  ...
  const selectorContext: OracleIRSelectorContext = {
    ...
    ...(hintChosenCreatureType || baseChosenCreatureType
      ...
      : {}),
    ...(hintChosenBasicLandType || baseChosenBasicLandType
      ? { chosenBasicLandType: hintChosenBasicLandType ?? baseChosenBasicLandType }
      : {}),
  ...
  if (!selectorContext.targetPlayerId && !selectorContext.targetOpponentId && !selectorContext.eachOfThoseOpponents) {
    if (
      ...
      ...(Number.isFinite(hintSpellCastCountThisTurn) ? { spellCastCountThisTurn: hintSpellCastCountThisTurn } : {}),
      ...(Number.isFinite(hintNoncreatureSpellCastCountThisTurn)
        ? { noncreatureSpellCastCountThisTurn: hintNoncreatureSpellCastCountThisTurn }
        : {}),
